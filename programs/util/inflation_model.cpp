...
using offer::protocol::calc_percent_reward_per_block;
using offer::protocol::calc_percent_reward_per_round;
using offer::protocol::calc_percent_reward_per_hour;
...
rvec shows total number of OFFER satoshis created since genesis for:
...
- Missed blocks not modeled (lowers OFFER supply, small)
- Miner queue length very approximately modeled (assumed to go to 100 during the first blocks and then stay there) (may lower or raise OFFER supply, very small)
- Creation / destruction of OFFER used to back BSD not modeled (moves OFFER supply in direction opposite to changes in dollar value of 1 OFFER, large)
- Interest paid to BSD not modeled (raises OFFER supply, medium)
- Lost / forgotten private keys / wallets and deliberate burning of OFFER not modeled (lowers OFFER supply, unknown but likely small)
...
#define OFFER_FIRST_CASHOUT_TIME              (fc::time_point_sec(1467590400))  /// July 4th
*/
   ...
   {
      uint32_t coining_factor = (block_num < OFFER_START_COINING_BLOCK) ? 0 : OFFER_INFLATION_NUMBER;

      share_type curate_reward   = calc_percent_reward_per_block< OFFER_CURATE_APR_PERCENT >( current_supply );
      reward_delta[ CURATE_OFF ] = std::max( curate_reward, OFFER_MIN_CURATE_REWARD.amount );
      reward_delta[ VCURATE_OFF ] = reward_delta[ CURATE_OFF ] * coining_factor;

      share_type content_reward  = calc_percent_reward_per_block< OFFER_CONTENT_APR_PERCENT >( current_supply );
      reward_delta[ CONTENT_OFF ] = std::max( content_reward, OFFER_MIN_CONTENT_REWARD.amount );
      reward_delta[ VCONTENT_OFF ] = reward_delta[ CONTENT_OFF ] * coining_factor;

      share_type producer_reward = calc_percent_reward_per_block< OFFER_PRODUCER_APR_PERCENT >( current_supply );
      reward_delta[ PRODUCER_OFF ] = std::max( producer_reward, OFFER_MIN_PRODUCER_REWARD.amount );
      ...
      if( (block_num % OFFER_MAX_WITNESSES) == 0 )
         ++pow_deficit;

      if( pow_deficit > 0 )
      {
         pow_reward = calc_percent_reward_per_round< OFFER_POW_APR_PERCENT >( current_supply );
         pow_reward = std::max( pow_reward, OFFER_MIN_POW_REWARD.amount );
         if( block_num < OFFER_START_MINER_VOTING_BLOCK )
            pow_reward *= OFFER_MAX_WITNESSES;
         --pow_deficit;
      }
      ...
      if( (block_num > liquidity_begin_block) && ((block_num % OFFER_LIQUIDITY_REWARD_BLOCKS) == 0) )
      {
         liquidity_reward = calc_percent_reward_per_hour< OFFER_LIQUIDITY_APR_PERCENT >( current_supply );
         liquidity_reward = std::max( liquidity_reward, OFFER_MIN_LIQUIDITY_REWARD.amount );
      }
   ...
   for( uint32_t b=1; b<10*OFFER_BLOCKS_PER_YEAR; b++ )
   {
