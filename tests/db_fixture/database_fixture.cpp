...
uint32_t OFFER_TESTING_GENESIS_TIMESTAMP = 1431700000;

using namespace offer::plugins::webserver;
using namespace offer::plugins::database_api;
using namespace offer::plugins::block_api;
using offer::plugins::condenser_api::condenser_api_plugin;
...
clean_database_fixture::clean_database_fixture()
{
   ...
   db_plugin = &appbase::app().register_plugin< offer::plugins::debug_node::debug_node_plugin >();
   ...
   appbase::app().initialize<
      offer::plugins::account_history::account_history_plugin,
      offer::plugins::debug_node::debug_node_plugin,
      offer::plugins::rc::rc_plugin,
      offer::plugins::witness::witness_plugin
      >( argc, argv );

   offer::plugins::rc::rc_plugin_skip_flags rc_skip;
   ...
   appbase::app().get_plugin< offer::plugins::rc::rc_plugin >().set_rc_plugin_skip_flags( rc_skip );

   db = &appbase::app().get_plugin< offer::plugins::chain::chain_plugin >().db();
   BOOST_REQUIRE( db );
   ...
   generate_block();
   db->set_hardfork( OFFER_BLOCKCHAIN_VERSION.minor() );
   generate_block();

   coin( "offerhare", 10000 );

   // Fill up the rest of the required miners
   for( int i = OFFER_NUM_INIT_MINERS; i < OFFER_MAX_WITNESSES; i++ )
   {
      account_create( OFFER_INIT_MINER_NAME + fc::to_string( i ), init_account_pub_key );
      fund( OFFER_INIT_MINER_NAME + fc::to_string( i ), OFFER_MIN_PRODUCER_REWARD.amount.value );
      witness_create( OFFER_INIT_MINER_NAME + fc::to_string( i ), init_account_priv_key, "foo.bar", init_account_pub_key, OFFER_MIN_PRODUCER_REWARD.amount );
   }
...
void clean_database_fixture::validate_database()
{
   ...
   appbase::app().get_plugin< offer::plugins::rc::rc_plugin >().validate_database();
}
...
void database_fixture::open_database()
{
   if( !data_dir )
   {
      data_dir = fc::temp_directory( offer::utilities::temp_directory_path() );
...
{
   skip |= default_skip;
   db_plugin->debug_generate_blocks( offer::utilities::key_to_wif( key ), 1, skip, miss_blocks );
}
...
void database_fixture::generate_blocks(fc::time_point_sec timestamp, bool miss_intermediate_blocks)
{
   ...
   BOOST_REQUIRE( ( db->head_block_time() - timestamp ).to_seconds() < OFFER_BLOCK_INTERVAL );
}
   ...
   )
{
   try
   {
      ...
      op.creator = creator;
      op.fee = asset( actual_fee, OFFER_SYMBOL );
      ...
      trx.set_expiration( db->head_block_time() + OFFER_MAX_TIME_UNTIL_EXPIRATION );
      ...
      if( fee_remainder > 0 )
      {
         coin( OFFER_INIT_MINER_NAME, name, asset( fee_remainder, OFFER_SYMBOL ) );
      }
...
)
{
   try
   {
      return account_create(
         name,
         OFFER_INIT_MINER_NAME,
         init_account_priv_key,
         std::max( db->get_witness_schedule_object().median_props.account_creation_fee.amount * OFFER_CREATE_ACCOUNT_WITH_OFFER_MODIFIER, share_type( 100 ) ),
         key,
         post_key,
         "" );
   }
   ...
   )
{
   try
   {
      transfer( OFFER_INIT_MINER_NAME, account_name, asset( amount, OFFER_SYMBOL ) );
      ...
      db_plugin->debug_update( [=]( database& db)
      {
         ...
         if( amount.symbol == BSD_SYMBOL )
         {
            ...
            if( median_feed.current_median_history.is_null() )
               ...
               {
                  f.current_median_history = price( asset( 1, BSD_SYMBOL ), asset( 1, OFFER_SYMBOL ) );
               });
         }
   ...
   const asset& amount )
{
   try
   {
      ...
      {
         db->adjust_balance( account_name, -amount );
         db->adjust_balance( account_name, db->to_offer( amount ) );
         db->adjust_supply( -amount );
         db->adjust_supply( db->to_offer( amount ) );
      }
   } FC_CAPTURE_AND_RETHROW( (account_name)(amount) )
}
...
void database_fixture::coin( const string& from, const string& to, const asset& amount )
{
   try
   {
      FC_ASSERT( amount.symbol == OFFER_SYMBOL, "Can only coin TESTS" );
...
void database_fixture::coin( const string& from, const share_type& amount )
{
   try
   {
      ...
      op.from = from;
      op.to = "";
      op.amount = asset( amount, OFFER_SYMBOL );
...
void database_fixture::set_price_feed( const price& new_price )
{
   for( size_t i = 1; i < 8; i++ )
   {
      ...
      op.owner = OFFER_INIT_MINER_NAME + fc::to_string( i );
   ...
   generate_blocks( OFFER_BLOCKS_PER_HOUR );
...
{
   trx.clear();
   for( size_t i=0; i<OFFER_MAX_WITNESSES; i++ )
   {
      witness_set_properties_operation op;
      op.owner = OFFER_INIT_MINER_NAME + (i == 0 ? "" : fc::to_string( i ));
      op.props = props;
   ...
   for( size_t i=0; i<2*OFFER_MAX_WITNESSES+1; i++ )
   {
      generate_block();
   ...
   FC_ASSERT( false, "Couldn't apply properties in ${n} blocks", ("n", 2*OFFER_MAX_WITNESSES+1) );
}
...
vector< operation > database_fixture::get_last_operations( uint32_t num_ops )
{
   ...
   while( itr != acc_hist_idx.begin() && ops.size() < num_ops )
   {
      ...
      ops.push_back( fc::raw::unpack_from_vector< offer::chain::operation >( serialized_op ) );
   }
   ...
   uint8_t token_decimal_places )
{
   ...
   try
   {
      ...
      tx.set_expiration( this->db->head_block_time() + OFFER_MAX_TIME_UNTIL_EXPIRATION );
...
void push_invalid_operation(const operation& invalid_op, const fc::ecc::private_key& key, database* db)
{
   ...
   OFFER_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::assert_exception );
}
...
void t_smt_database_fixture< T >::create_invalid_smt( const char* control_account_name, const fc::ecc::private_key& key )
{
   ...
   OFFER_REQUIRE_THROW( set_create_op(this->db, &op_precision, control_account_name, OFFER_ASSET_MAX_DECIMALS + 1), fc::assert_exception );
}
...
template< typename T >
smt_generation_unit t_smt_database_fixture< T >::get_generation_unit( const units& offer_unit, const units& token_unit )
{
   smt_generation_unit ret;

   ret.offer_unit = offer_unit;
...
smt_cap_commitment t_smt_database_fixture< T >::get_cap_commitment( share_type amount, uint128_t nonce )
{
   ...
   else
   {
      ...
      ret.lower_bound = SMT_MIN_HARD_CAP_OFFER_UNITS; // See smt_capped_generation_policy::validate
      ret.upper_bound = OFFER_MAX_SHARE_SUPPLY/10;    // See smt_capped_generation_policy::validate
   }
...
(
   ...
   const smt_cap_commitment& min_offer_units_commitment,
   const smt_cap_commitment& hard_cap_offer_units_commitment,
...
)
{
   ...
   ret.min_offer_units_commitment = min_offer_units_commitment;
   ret.hard_cap_offer_units_commitment = hard_cap_offer_units_commitment;
...
template smt_generation_unit t_smt_database_fixture< clean_database_fixture >::get_generation_unit( const units& offer_unit, const units& token_unit );
...
json_rpc_database_fixture::json_rpc_database_fixture()
{
   ...
   db_plugin = &appbase::app().register_plugin< offer::plugins::debug_node::debug_node_plugin >();
   appbase::app().register_plugin< offer::plugins::witness::witness_plugin >();
   rpc_plugin = &appbase::app().register_plugin< offer::plugins::json_rpc::json_rpc_plugin >();
   ...
   appbase::app().initialize<
      ...
      offer::plugins::block_api::block_api_plugin,
      offer::plugins::database_api::database_api_plugin,
      offer::plugins::condenser_api::condenser_api_plugin
      >( argc, argv );

   appbase::app().get_plugin< offer::plugins::condenser_api::condenser_api_plugin >().plugin_startup();
