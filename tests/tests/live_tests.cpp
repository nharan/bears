...
BOOST_AUTO_TEST_CASE( coins_stock_split )
{
   try
   {
      ...
      auto old_coining_fund = db->get_dynamic_global_properties().total_coining_fund_offer;
      ...
      auto old_reward_fund = db->get_dynamic_global_properties().total_reward_fund_offer;
      ...
      while( com_itr != com_idx.end() )
      {
         ...
         if( com_itr->net_rshares.value > 0 )
         {
            ...
            u256 rf( gpo.total_reward_fund_offer.amount.value );
            auto rs2 = rs * rs;
      ...
      BOOST_REQUIRE( db->get_dynamic_global_properties().total_coining_fund_offer == old_coining_fund );
      ...
      BOOST_REQUIRE( db->get_dynamic_global_properties().total_reward_fund_offer == old_reward_fund );
