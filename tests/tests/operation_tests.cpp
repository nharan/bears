...
#include <offer/plugins/witness/witness_objects.hpp>
...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      const account_object& init = db->get_account( OFFER_INIT_MINER_NAME );
      asset init_starting_balance = init.balance;

      account_create_operation op;

      op.new_account_name = "alice";
      op.creator = OFFER_INIT_MINER_NAME;
      ...
      tx.signatures.clear();
      tx.operations.clear();
      op.fee = asset( db->get_account( OFFER_INIT_MINER_NAME ).balance.amount + 1, OFFER_SYMBOL );
      op.new_account_name = "bob";
      tx.operations.push_back( op );
      sign( tx, init_account_priv_key );
      ...
      fund( OFFER_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );
      coin( OFFER_INIT_MINER_NAME, OFFER_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );

      BOOST_TEST_MESSAGE( "--- Test account creation with temp account does not set recovery account" );
      op.creator = OFFER_TEMP_ACCOUNT;
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
   try
   {
      ...
      OFFER_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_active_auth );
      ...
      sign( tx, alice_private_key );
      OFFER_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when containing duplicate signatures" );
      tx.signatures.clear();
      sign( tx, active_key );
      sign( tx, active_key );
      OFFER_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_duplicate_sig );
      ...
      tx.operations.push_back( op );
      sign( tx, active_key );
      OFFER_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
   try
   {
      ...
      ACTORS( (alice)(bob) );
      generate_blocks( 60 / OFFER_BLOCK_INTERVAL );
      ...
      OFFER_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_posting_auth );
      ...
      sign( tx, bob_private_key );
      OFFER_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when signed by a signature not in the creator's authority" );
      tx.signatures.clear();
      sign( tx, bob_private_key );
      OFFER_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db->head_block_time() + fc::seconds( OFFER_CASHOUT_WINDOW_SECONDS ) ) );
      ...
      BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + OFFER_CASHOUT_WINDOW_SECONDS );
      ...
      BOOST_REQUIRE( sam_comment.cashout_time == sam_comment.created + OFFER_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( sam_comment.root_comment == alice_comment.id );
      validate_database();

      generate_blocks( 60 * 5 / OFFER_BLOCK_INTERVAL + 1 );
      ...
      {
         o.total_reward_shares2 = offer::chain::util::evaluate_reward_curve( 10 );
      });
      ...
      BOOST_REQUIRE( mod_sam_comment.cashout_time == mod_sam_comment.created + OFFER_CASHOUT_WINDOW_SECONDS );
      validate_database();
      ...
      generate_blocks( 60 * 5 / OFFER_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_delete_apply )
{
   try
   {
      ...
      coin( OFFER_INIT_MINER_NAME, "alice", ASSET( "1000.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + OFFER_MIN_TRANSACTION_EXPIRATION_LIMIT );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      ...
      generate_block();
      vote.weight = -1 * OFFER_100_PERCENT;
      tx.clear();
      tx.operations.push_back( vote );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      generate_blocks( OFFER_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / OFFER_BLOCK_INTERVAL );
      ...
      generate_blocks( OFFER_CASHOUT_WINDOW_SECONDS / OFFER_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         int64_t max_vote_denom = ( db->get_dynamic_global_properties().vote_power_reserve_rate * OFFER_VOTING_MANA_REGENERATION_SECONDS ) / (60*60*24);

         BOOST_REQUIRE( alice.last_vote_time == db->head_block_time() );
         BOOST_REQUIRE( alice_comment.net_rshares.value == ( old_mana - alice.voting_manabar.current_mana ) - OFFER_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + OFFER_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == ( old_mana - alice.voting_manabar.current_mana ) - OFFER_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();

         BOOST_TEST_MESSAGE( "--- Test reduced power for quick voting" );

         generate_blocks( db->head_block_time() + OFFER_MIN_VOTE_INTERVAL_SEC );

         util::manabar old_manabar = db->get_account( "alice" ).voting_manabar;
         util::manabar_params params( util::get_effective_coining_shares( db->get_account( "alice" ) ), OFFER_VOTING_MANA_REGENERATION_SECONDS );
         ...
         BOOST_REQUIRE( bob_comment.net_rshares.value == ( old_manabar.current_mana - db->get_account( "alice" ).voting_manabar.current_mana ) - OFFER_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + OFFER_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         generate_blocks( db->head_block_time() + fc::seconds( ( OFFER_CASHOUT_WINDOW_SECONDS / 2 ) ), true );
         ...
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + ( old_mana - new_bob.voting_manabar.current_mana ) - OFFER_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( new_alice_comment.cashout_time == new_alice_comment.created + OFFER_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         op.weight = -1 * OFFER_100_PERCENT / 2;
         op.voter = "sam";
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         ...
         int64_t sam_weight = old_manabar.current_mana - db->get_account( "sam" ).voting_manabar.current_mana - OFFER_VOTE_DUST_THRESHOLD;
         ...
         BOOST_REQUIRE( new_bob_comment.cashout_time == new_bob_comment.created + OFFER_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         int64_t regenerated_power = (OFFER_100_PERCENT * ( db->head_block_time() - db->get_account( "alice").last_vote_time ).to_seconds() ) / OFFER_VOTING_MANA_REGENERATION_SECONDS;
         ...
         int64_t new_rshares = ( ( fc::uint128_t( db->get_account( "alice" ).coining_shares.amount.value ) * used_power ) / OFFER_100_PERCENT ).to_uint64() - OFFER_VOTE_DUST_THRESHOLD;

         BOOST_REQUIRE( db->get_comment( "alice", string( "foo" ) ).cashout_time == db->get_comment( "alice", string( "foo" ) ).created + OFFER_CASHOUT_WINDOW_SECONDS );
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( ( OFFER_1_PERCENT * 25 * ( get_voting_power( new_alice ) ) / OFFER_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "alice" ).voting_manabar.current_mana - OFFER_VOTE_DUST_THRESHOLD;
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( uint64_t( OFFER_1_PERCENT ) * 75 * uint64_t( alice_voting_power ) ) / OFFER_100_PERCENT;
         ...
         generate_blocks( fc::time_point_sec( ( new_bob_comment.cashout_time - OFFER_UPVOTE_LOCKOUT_HF17 ).sec_since_epoch() + OFFER_BLOCK_INTERVAL ), true );
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "dave" ).voting_manabar.current_mana - OFFER_VOTE_DUST_THRESHOLD;
         new_rshares = ( new_rshares * ( OFFER_UPVOTE_LOCKOUT_SECONDS - OFFER_BLOCK_INTERVAL ) ) / OFFER_UPVOTE_LOCKOUT_SECONDS;
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "dave" ).voting_manabar.current_mana - OFFER_VOTE_DUST_THRESHOLD;
         new_rshares = ( new_rshares * ( OFFER_UPVOTE_LOCKOUT_SECONDS - OFFER_BLOCK_INTERVAL - OFFER_BLOCK_INTERVAL ) ) / OFFER_UPVOTE_LOCKOUT_SECONDS;
...
BOOST_AUTO_TEST_CASE( transfer_apply )
{
   try
   {
      ...
      tx.set_expiration( db->head_block_time() + OFFER_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      OFFER_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::exception );
...
BOOST_AUTO_TEST_CASE( transfer_to_coining_apply )
{
   try
   {
      ...
      auto coins = asset( gpo.total_coining_fund_offer.amount, OFFER_SYMBOL );
      ...
      BOOST_REQUIRE( gpo.total_coining_fund_offer.amount.value == coins.amount.value );
      BOOST_REQUIRE( gpo.total_coining_shares.amount.value == shares.amount.value );
      validate_database();

      op.to = "bob";
      op.amount = asset( 2000, OFFER_SYMBOL );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + OFFER_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( withdraw_coining_authorities )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure when no signature." );
      OFFER_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_active_auth );
      ...
      sign( tx, alice_private_key );
      OFFER_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_duplicate_sig );
...
BOOST_AUTO_TEST_CASE( withdraw_coining_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.coining_withdraw_rate.amount.value == ( old_coining_shares.amount / ( OFFER_COINING_WITHDRAW_INTERVALS * 2 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.coining_shares.amount.value );
      BOOST_REQUIRE( alice.next_coining_withdrawal == db->head_block_time() + OFFER_COINING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      BOOST_REQUIRE( alice.coining_shares.amount.value == old_coining_shares.amount.value );
      BOOST_REQUIRE( alice.coining_withdraw_rate.amount.value == ( old_coining_shares.amount / ( OFFER_COINING_WITHDRAW_INTERVALS * 3 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.coining_shares.amount.value );
      BOOST_REQUIRE( alice.next_coining_withdrawal == db->head_block_time() + OFFER_COINING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         {
            gpo.current_supply += wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" ) - gpo.total_coining_fund_offer;
            gpo.total_coining_fund_offer = wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" );
         });
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 TESTS" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = legacy_offer_asset::from_asset( asset(OFFER_MIN_ACCOUNT_CREATION_FEE + 10, OFFER_SYMBOL) );
      op.props.maximum_block_size = OFFER_MIN_BLOCK_SIZE_LIMIT + 100;
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.proxy == OFFER_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxy == OFFER_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxied_vsf_votes_total() == bob.coining_shares.amount );
      BOOST_REQUIRE( dave.proxy == OFFER_PROXY_TO_SELF_ACCOUNT );
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = OFFER_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( alice.proxy == "sam" );
      BOOST_REQUIRE( alice.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( bob.proxy == OFFER_PROXY_TO_SELF_ACCOUNT );
      BOOST_REQUIRE( bob.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( sam.proxy == "dave" );
      ...
      vote.account= "bob";
      vote.witness = OFFER_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( vote );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( db->get_witness( OFFER_INIT_MINER_NAME ).votes == ( alice.coining_shares + bob.coining_shares ).amount );
      validate_database();
      ...
      BOOST_REQUIRE( db->get_witness( OFFER_INIT_MINER_NAME ).votes == bob.coining_shares.amount );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_apply )
{
   try
   {
      ...
      feed_publish_operation op;
      op.publisher = "alice";
      op.exchange_rate = price( ASSET( "1.000 TBD" ), ASSET( "1000.000 TESTS" ) ); // 1000 OFFER : 1 BSD
...
BOOST_AUTO_TEST_CASE( convert_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( convert_request->conversion_date == db->head_block_time() + OFFER_CONVERSION_DELAY );
...
BOOST_AUTO_TEST_CASE( fixture_convert_checks_balance )
{
   ...
   try
   {
      ...
      fund( "dany", 5000 );
      OFFER_REQUIRE_THROW( convert( "dany", ASSET( "5000.000 TESTS" ) ), fc::exception );
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( limit_order_create_apply )
{
   try
   {
      ...
      op.expiration = db->head_block_time() + fc::seconds( OFFER_MAX_LIMIT_ORDER_EXPIRATION + 1 );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( BSD_SYMBOL, OFFER_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( limit_order_create2_apply )
{
   try
   {
      ...
      /// First check validation on price constructor level:
      {
        price broken_price;
        /// Invalid base value
        OFFER_REQUIRE_THROW(broken_price=price(ASSET("0.000 TESTS"), ASSET("1.000 TBD")),
          fc::exception);
        /// Invalid quote value
        OFFER_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TBD")),
          fc::exception);
        /// Invalid symbol (same in base & quote)
        OFFER_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TESTS")),
          fc::exception);
      }
...
BOOST_AUTO_TEST_CASE( limit_order_cancel_authorities )
{
   try
   {
      ...
      c.expiration = db->head_block_time() + fc::seconds( OFFER_MAX_LIMIT_ORDER_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( limit_order_cancel_apply )
{
   try
   {
      ...
      create.expiration = db->head_block_time() + fc::seconds( OFFER_MAX_LIMIT_ORDER_EXPIRATION );
      tx.operations.clear();
      tx.signatures.clear();
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      coin( OFFER_INIT_MINER_NAME, "bob", asset( 1000, OFFER_SYMBOL ) );
      ...
      generate_blocks( db->head_block_time() + OFFER_OWNER_UPDATE_LIMIT );
      ...
      generate_blocks( db->head_block_time() + OFFER_OWNER_UPDATE_LIMIT + fc::seconds( OFFER_BLOCK_INTERVAL ) );
      ...
      BOOST_REQUIRE( req_itr->expires == db->head_block_time() + OFFER_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - OFFER_BLOCK_INTERVAL ), true );
      ...
      generate_blocks( db->head_block_time() + ( OFFER_OWNER_AUTH_RECOVERY_PERIOD - OFFER_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      OFFER_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
      OFFER_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
      OFFER_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
      ...
      generate_blocks( db->head_block_time() + OFFER_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( OFFER_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      OFFER_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      OFFER_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      op.offer_amount = ASSET( "1.000 TESTS" );
      op.escrow_id = 0;
      op.agent = "sam";
      op.fee = ASSET( "0.100 TESTS" );
      op.json_meta = "";
      ...
      op.bsd_amount.symbol = OFFER_SYMBOL;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when offer symbol != OFFER" );
      op.bsd_amount.symbol = BSD_SYMBOL;
      op.offer_amount.symbol = BSD_SYMBOL;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when fee symbol != BSD and fee symbol != OFFER" );
      op.offer_amount.symbol = OFFER_SYMBOL;
      op.fee.symbol = COINS_SYMBOL;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when bsd == 0 and offer == 0" );
      op.fee.symbol = OFFER_SYMBOL;
      op.bsd_amount.amount = 0;
      op.offer_amount.amount = 0;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when bsd < 0" );
      op.bsd_amount.amount = -100;
      op.offer_amount.amount = 1000;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when offer < 0" );
      op.bsd_amount.amount = 1000;
      op.offer_amount.amount = -100;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_transfer_apply )
{
   try
   {
      ...
      auto alice_offer_balance = alice.balance - op.offer_amount - op.fee;
      auto alice_bsd_balance = alice.bsd_balance - op.bsd_amount;
      auto bob_offer_balance = bob.balance;
      auto bob_bsd_balance = bob.bsd_balance;
      auto sam_offer_balance = sam.balance;
      ...
      BOOST_REQUIRE( escrow.offer_balance == op.offer_amount );
      ...
      BOOST_REQUIRE( alice.balance == alice_offer_balance );
      BOOST_REQUIRE( alice.bsd_balance == alice_bsd_balance );
      BOOST_REQUIRE( bob.balance == bob_offer_balance );
      BOOST_REQUIRE( bob.bsd_balance == bob_bsd_balance );
      BOOST_REQUIRE( sam.balance == sam_offer_balance );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
   try
   {
      ...
      et_op.to = "bob";
      et_op.agent = "sam";
      et_op.offer_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_REQUIRE( escrow.offer_balance == ASSET( "1.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + OFFER_BLOCK_INTERVAL );
      sign( tx, bob_private_key );
      OFFER_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      ...
      OFFER_REQUIRE_THROW( db->get_escrow( op.from, op.escrow_id ), fc::exception );
      BOOST_REQUIRE( alice.balance == ASSET( "10.000 TESTS" ) );
      validate_database();
      ...
      generate_blocks( et_op.ratification_deadline + OFFER_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_dispute_apply )
{
   try
   {
      ...
      et_op.offer_amount = ASSET( "1.000 TESTS" );
      et_op.fee = ASSET( "0.100 TESTS" );
      et_op.ratification_deadline = db->head_block_time() + OFFER_BLOCK_INTERVAL;
      et_op.escrow_expiration = db->head_block_time() + 2 * OFFER_BLOCK_INTERVAL;
      ...
      BOOST_REQUIRE( escrow.offer_balance == et_op.offer_amount );
...
BOOST_AUTO_TEST_CASE( escrow_release_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when offer == 0 and bsd == 0" );
      op.bsd_amount.amount = 0;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when offer is not offer symbol" );
      op.bsd_amount.symbol = BSD_SYMBOL;
      op.offer_amount = ASSET( "1.000 TBD" );
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
   try
   {
      ...
      op.offer_amount = ASSET( "0.100 TESTS" );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).offer_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).offer_balance == ASSET( "0.800 TESTS" ) );
      ...
      BOOST_TEST_MESSAGE( "--- failure when releasing less offer than available" );
      op.offer_amount = ASSET( "0.000 TESTS" );
      op.bsd_amount = ASSET( "1.000 TBD" );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).offer_balance == ASSET( "0.700 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).offer_balance == ASSET( "0.600 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).offer_balance == ASSET( "0.500 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- success deleting escrow when balances are both zero" );
      tx.clear();
      op.offer_amount = ASSET( "0.500 TESTS" );
      tx.operations.push_back( op );
      sign( tx, sam_private_key );
      db->push_transaction( tx, 0 );

      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "9.700 TESTS" ) );
      OFFER_REQUIRE_THROW( db->get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).offer_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).offer_balance == ASSET( "0.800 TESTS" ) );
      ...
      tx.clear();
      op.offer_amount = ASSET( "0.600 TESTS" );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "success when amount is OFFER" );
      op.amount = ASSET( "1.000 TESTS" );
      op.validate();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- success transferring OFFER to self" );
      op.to = "alice";
      ...
      BOOST_TEST_MESSAGE( "--- success transferring OFFER to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing OFFER to self" );
      op.to = "alice";
      ...
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).complete == db->head_block_time() + OFFER_SAVINGS_WITHDRAW_TIME );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing OFFER to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id = 3;
      ...
      generate_blocks( db->head_block_time() + OFFER_SAVINGS_WITHDRAW_TIME - fc::seconds( OFFER_BLOCK_INTERVAL ), true );
      ...
      for( int i = 0; i < OFFER_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
      {
         op.request_id = i;
         tx.clear();
         tx.operations.push_back( op );
      ...
      op.request_id = OFFER_SAVINGS_WITHDRAW_REQUEST_LIMIT;
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      OFFER_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == OFFER_SAVINGS_WITHDRAW_REQUEST_LIMIT );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( itr->effective_date == db->head_block_time() + OFFER_OWNER_AUTH_RECOVERY_PERIOD );
      ...
      vote.weight = OFFER_1_PERCENT * 50;
      tx.clear();
      tx.operations.push_back( vote );
      sign( tx, alice_private_key );
      OFFER_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
...
BOOST_AUTO_TEST_CASE( account_bandwidth )
{
   try
   {
      ...
      BOOST_REQUIRE( average_bandwidth == fc::raw::pack_size( tx ) * 10 * OFFER_BANDWIDTH_PRECISION );
      ...
      BOOST_REQUIRE( last_bandwidth_update == db->head_block_time() );
      BOOST_REQUIRE( average_bandwidth == total_bandwidth + fc::raw::pack_size( tx ) * 10 * OFFER_BANDWIDTH_PRECISION );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "Testing wrong OFFER symbol" );
      op.reward_offer = ASSET( "1.000 TBD" );
      OFFER_REQUIRE_THROW( op.validate(), fc::assert_exception );


      BOOST_TEST_MESSAGE( "Testing wrong BSD symbol" );
      op.reward_offer = ASSET( "1.000 TESTS" );
   ...
   try
   {
      ...
      db_plugin->debug_update( []( database& db )
      {
         ...
         db.modify( db.get_dynamic_global_properties(), []( dynamic_global_property_object& gpo )
         {
            ...
            gpo.pending_rewarded_coining_offer += ASSET( "10.000 TESTS" );
         });
      });
      ...
      auto alice_offer = db->get_account( "alice" ).balance;
      auto alice_bsd = db->get_account( "alice" ).bsd_balance;
      auto alice_coins = db->get_account( "alice" ).coining_shares;


      BOOST_TEST_MESSAGE( "--- Attempting to claim more OFFER than exists in the reward balance." );

      claim_reward_balance_operation op;
      signed_transaction tx;

      op.account = "alice";
      op.reward_offer = ASSET( "20.000 TESTS" );
      op.reward_bsd = ASSET( "0.000 TBD" );
      op.reward_coins = ASSET( "0.000000 COINS" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_offer + op.reward_offer );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_offer_balance == ASSET( "10.000 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).reward_coining_offer == ASSET( "5.000 TESTS" ) );
      validate_database();
      ...
      op.reward_offer = ASSET( "10.000 TESTS" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_offer + op.reward_offer );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_offer_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).bsd_balance == alice_bsd + op.reward_bsd );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).reward_coining_offer == ASSET( "0.000 TESTS" ) );
            validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( delegate_coining_shares_authorities )
{
   try
   {
      ...
      signed_transaction tx;
      ACTORS( (alice)(bob) )
      coin( OFFER_INIT_MINER_NAME, "alice", ASSET( "10000.000 TESTS" ) );
...
BOOST_AUTO_TEST_CASE( delegate_coining_shares_apply )
{
   try
   {
      ...
      util::manabar_params params( util::get_effective_coining_shares( db->get_account( "bob" ) ), OFFER_VOTING_MANA_REGENERATION_SECONDS );
      old_manabar.regenerate_mana( params, db->head_block_time() );
      ...
      BOOST_REQUIRE( alice_comment.net_rshares.value == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - OFFER_VOTE_DUST_THRESHOLD );
      BOOST_REQUIRE( itr->rshares == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - OFFER_VOTE_DUST_THRESHOLD );

      generate_block();
      ACTORS( (sam)(dave) )
      generate_block();

      coin( OFFER_INIT_MINER_NAME, "sam", ASSET( "1000.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + OFFER_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, sam_private_key );
      OFFER_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      BOOST_REQUIRE( gpo.delegation_return_period == OFFER_DELEGATION_RETURN_PERIOD_HF20 );
      ...
      generate_blocks( exp_obj->expiration + OFFER_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_validate )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), OFFER_100_PERCENT + 1 ) );
      op.extensions.insert( b );
      OFFER_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- Testing more than 100% total weight" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), OFFER_1_PERCENT * 75 ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), OFFER_1_PERCENT * 75 ) );
      op.extensions.clear();
      op.extensions.insert( b );
      OFFER_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", OFFER_1_PERCENT * 2 ) );
      b.beneficiaries.push_back( beneficiary_route_type( "bob", OFFER_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      OFFER_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", OFFER_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( "alice", OFFER_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      OFFER_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), OFFER_1_PERCENT ) );

      for( size_t i = 0; i < 8; i++ )
      {
         b.beneficiaries.push_back( beneficiary_route_type( account_name_type( OFFER_INIT_MINER_NAME + fc::to_string( i ) ), OFFER_1_PERCENT ) );
      }
      ...
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      OFFER_REQUIRE_THROW( db->push_transaction( tx ), chain::plugin_exception );


      BOOST_TEST_MESSAGE( "--- Test specifying a non-existent benefactor" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "doug" ), OFFER_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.clear();
      tx.operations.push_back( op );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), 25 * OFFER_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), 50 * OFFER_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "dave" ), 25 * OFFER_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      sign( tx, alice_private_key );
      OFFER_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - OFFER_BLOCK_INTERVAL );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [=]( dynamic_global_property_object& gpo )
         {
            gpo.current_supply -= gpo.total_reward_fund_offer;
            gpo.total_reward_fund_offer = ASSET( "100.000 TESTS" );
            gpo.current_supply += gpo.total_reward_fund_offer;
         });
      });
      ...
      BOOST_REQUIRE( db->get_account( "bob" ).reward_coining_offer.amount + db->get_account( "bob" ).reward_bsd_balance.amount + db->get_account( "sam" ).reward_coining_offer.amount + db->get_account( "sam" ).reward_bsd_balance.amount == db->get_comment( "alice", string( "test" ) ).beneficiary_payout_value.amount );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_bsd_balance.amount + db->get_account( "alice" ).reward_coining_offer.amount ) == db->get_account( "bob" ).reward_coining_offer.amount + db->get_account( "bob" ).reward_bsd_balance.amount + 2 );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_bsd_balance.amount + db->get_account( "alice" ).reward_coining_offer.amount ) * 2 == db->get_account( "sam" ).reward_coining_offer.amount + db->get_account( "sam" ).reward_bsd_balance.amount + 3 );
      BOOST_REQUIRE( db->get_account( "bob" ).reward_coining_offer.amount == db->get_account( "bob" ).reward_bsd_balance.amount );
      BOOST_REQUIRE( db->get_account( "sam" ).reward_coining_offer.amount == db->get_account( "sam" ).reward_bsd_balance.amount + 1 );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_validate )
{
   try
   {
      ...
      prop_op.owner = "alice";
      OFFER_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting maximum_block_size below OFFER_MIN_BLOCK_SIZE_LIMIT" );
      prop_op.props.erase( "account_creation_fee" );
      prop_op.props[ "maximum_block_size" ] = fc::raw::pack_to_vector( OFFER_MIN_BLOCK_SIZE_LIMIT - 1 );
      OFFER_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting bsd_interest_rate to OFFER_100_PERCENT + 1" );
      prop_op.props[ "bsd_interest_rate" ].clear();
      prop_op.props[ "bsd_interest_rate" ] = fc::raw::pack_to_vector( OFFER_100_PERCENT + 1 );
      OFFER_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting new bsd_exchange_rate with BSD / OFFER" );
      ...
      uint64_t unit = uint64_t(1) << OFFER_RD_DECAY_DENOM_SHIFT;

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one year" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / OFFER_BLOCKS_PER_YEAR ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one day" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / OFFER_BLOCKS_PER_DAY ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one hour" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / ((60*60)/OFFER_BLOCK_INTERVAL) ) );
      prop_op.validate();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_authorities )
{
   try
   {
      ...
      expected_keys.push_back( authority( 1, OFFER_NULL_ACCOUNT, 1 ) );
      op.get_required_authorities( key_auths );
      BOOST_REQUIRE( key_auths == expected_keys );
...
BOOST_AUTO_TEST_CASE( witness_set_properties_apply )
{
   try
   {
      ...
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.maximum_block_size == OFFER_MIN_BLOCK_SIZE_LIMIT + 1 );
      ...
      prop_op.props[ "key" ].clear();
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "account_subsidy_budget" ] = fc::raw::pack_to_vector( OFFER_ACCOUNT_SUBSIDY_PRECISION );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.account_subsidy_budget == OFFER_ACCOUNT_SUBSIDY_PRECISION );

      BOOST_TEST_MESSAGE( "--- Testing setting account subsidy pool cap" );
      uint64_t day_decay = ( uint64_t(1) << OFFER_RD_DECAY_DENOM_SHIFT ) / OFFER_BLOCKS_PER_DAY;
...
BOOST_AUTO_TEST_CASE( claim_account_apply )
{
   try
   {
      ...
      generate_blocks( OFFER_BLOCKS_PER_HOUR/2 );
      ...
      BOOST_REQUIRE( db->get_account( OFFER_NULL_ACCOUNT ).balance == ASSET( "5.000 TESTS" ) );
      ...
      BOOST_CHECK_EQUAL( c_subs, prev_c_subs - OFFER_ACCOUNT_SUBSIDY_PRECISION );
      BOOST_CHECK_EQUAL( nc_subs, prev_nc_subs );
      ...
      int64_t new_value = prev_c_subs - OFFER_ACCOUNT_SUBSIDY_PRECISION;     // Usage applied before decay
      new_value = new_value
          + 5000                                                             // Budget
          - ((new_value*249617279) >> OFFER_RD_DECAY_DENOM_SHIFT);           // Decay
...
BOOST_AUTO_TEST_CASE( create_claimed_account_apply )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( OFFER_TEMP_ACCOUNT ), [&]( account_object& a )
         {
