...
#if defined IS_TEST_NET && defined OFFER_ENABLE_SMT
...
BOOST_AUTO_TEST_CASE( smt_create_validate )
{
   try
   {
      ...
      op.smt_creation_fee.amount = OFFER_MAX_SHARE_SUPPLY;
      op.validate();
...
BOOST_AUTO_TEST_CASE( smt_create_apply )
{
   try
   {
      ...
      // Fund with OFFER, and set fee with BSD.
      ...
      // Now fund with BSD, and set fee with OFFER.
      convert( "alice", asset( test_amount, OFFER_SYMBOL ) );
      // Declare fee in OFFER though alice has none.
      op.smt_creation_fee = asset( test_amount, OFFER_SYMBOL );
      // Throw due to insufficient balance of OFFER.
      FAIL_WITH_OP(op, alice_private_key, fc::assert_exception);
      ...
      // Check too low fee in OFFER.
      FUND( "bob", too_low_fee_amount );
      op.smt_creation_fee = asset( too_low_fee_amount, OFFER_SYMBOL );
      FAIL_WITH_OP(op, bob_private_key, fc::assert_exception);

      // Check too low fee in BSD.
      convert( "bob", asset( too_low_fee_amount, OFFER_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( setup_emissions_validate )
{
   try
   {
      ...
      op.control_account = "alice";
      // schedule_time <= OFFER_GENESIS_TIME;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( set_setup_parameters_validate )
{
   try
   {
      ...
      OFFER_REQUIRE_THROW( op.validate(), fc::exception ); // invalid symbol
      op.symbol = dany_symbol;

      op.control_account = "####";
      OFFER_REQUIRE_THROW( op.validate(), fc::exception ); // invalid account name
...
BOOST_AUTO_TEST_CASE( setup_emissions_apply )
{
   try
   {
      ...
      auto smts = create_smt_3("alice", alice_private_key);
      {
         ...
         const offer::chain::smt_token_object* smt = db->find< offer::chain::smt_token_object, by_symbol >( fail_op.symbol );
         FC_ASSERT( smt != nullptr, "The SMT has just been created!" );
         FC_ASSERT( smt->phase < offer::chain::smt_phase::setup_completed, "Who closed setup phase?!" );
         db->modify( *smt, [&]( offer::chain::smt_token_object& token )
         {
            token.phase = offer::chain::smt_phase::setup_completed;
         });
...
BOOST_AUTO_TEST_CASE( comment_votable_assers_validate )
{
   try
   {
...

      {
         ...
         BOOST_TEST_MESSAGE( "--- Testing invalid configuration of votable_assets - OFFER added to container" );
         allowed_vote_assets ava;
         const auto& smt = smts.front();
         ava.add_votable_asset(smt, share_type(20), false);
         ava.add_votable_asset(OFFER_SYMBOL, share_type(20), true);
         op.extensions.insert( ava );
         OFFER_REQUIRE_THROW( op.validate(), fc::assert_exception );
      }
   }
...
BOOST_AUTO_TEST_CASE( cap_commit_reveal_validate )
{
   try
   {
      ...
      revealMaxPlus.amount = OFFER_MAX_SHARE_SUPPLY + 1;
      ...
      OFFER_REQUIRE_THROW( public_cap0.validate(), fc::assert_exception );
      // Test cap value too big
      smt_cap_commitment public_capMaxPlus;
      public_capMaxPlus.lower_bound = public_capMaxPlus.upper_bound = OFFER_MAX_SHARE_SUPPLY + 1;
      public_capMaxPlus.hash = fc::sha256::hash( revealMaxPlus );
      OFFER_REQUIRE_THROW( public_capMaxPlus.validate(), fc::assert_exception );
      ...
      // Test reveal too low
      OFFER_REQUIRE_THROW( reveal1K1234.validate(hidden_cap1M1234), fc::assert_exception );
      // Test reveal too big
      OFFER_REQUIRE_THROW( reveal1G1234.validate(hidden_cap1M1234), fc::assert_exception );
      // Test wrong nonce
      OFFER_REQUIRE_THROW( reveal1M4321.validate(hidden_cap1M1234), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( asset_symbol_coining_methods )
{
   try
   {
      ...
      asset_symbol_type Offer = OFFER_SYMBOL;
      FC_ASSERT( Offer.is_coining() == false );
      FC_ASSERT( Offer.get_paired_symbol() == COINS_SYMBOL );

      asset_symbol_type Coins = COINS_SYMBOL;
      FC_ASSERT( Coins.is_coining() );
      FC_ASSERT( Coins.get_paired_symbol() == OFFER_SYMBOL );
...
BOOST_AUTO_TEST_CASE( setup_validate )
{
   try
   {
      ...
      //FC_ASSERT( max_supply <= OFFER_MAX_SHARE_SUPPLY )
      op.max_supply = OFFER_MAX_SHARE_SUPPLY + 1;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( generation_begin_time > OFFER_GENESIS_TIME )
      op.max_supply = OFFER_MAX_SHARE_SUPPLY / 1000;
      op.generation_begin_time = OFFER_GENESIS_TIME;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      (
         ...
         get_cap_commitment( 1 )/*min_offer_units_commitment*/,
         get_cap_commitment( SMT_MIN_HARD_CAP_OFFER_UNITS + 1 )/*hard_cap_offer_units_commitment*/,
         OFFER_100_PERCENT/*soft_cap_percent*/,
         1/*min_unit_ratio*/,
         2/*max_unit_ratio*/
      );
      ...
      //FC_ASSERT( offer_unit.size() <= SMT_MAX_UNIT_ROUTES )
      gp.pre_soft_cap_unit.offer_unit = to_many_units;
      gp.pre_soft_cap_unit.token_unit = { { "bob",3 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.offer_unit = { { "bob2", 33 } };
      gp.pre_soft_cap_unit.token_unit = to_many_units;
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //Invalid account
      gp.pre_soft_cap_unit.offer_unit = { { "{}{}", 12 } };
      gp.pre_soft_cap_unit.token_unit = { { "xyz", 13 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.offer_unit = { { "xyz2", 14 } };
      gp.pre_soft_cap_unit.token_unit = { { "{}", 15 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //Invalid account -> valid is '$from'
      gp.pre_soft_cap_unit.offer_unit = { { "$fromx", 1 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from", 2 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.offer_unit = { { "$from", 3 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from_", 4 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //Invalid account -> valid is '$from.coining'
      gp.pre_soft_cap_unit.offer_unit = { { "$from.coiningx", 2 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from.coining", 222 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.offer_unit = { { "$from.coining", 13 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from.coining.coining", 3 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( offer_unit.value > 0 );
      gp.pre_soft_cap_unit.offer_unit = { { "$from.coining", 0 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from.coining", 2 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.offer_unit = { { "$from.coining", 10 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from.coining", 0 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( offer_unit.value > 0 );
      gp.pre_soft_cap_unit.offer_unit = { { "$from", 0 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from", 100 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.offer_unit = { { "$from", 33 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from", 0 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( offer_unit.value > 0 );
      gp.pre_soft_cap_unit.offer_unit = { { "qprst", 0 } };
      gp.pre_soft_cap_unit.token_unit = { { "qprst", 67 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.offer_unit = { { "my_account2", 55 } };
      gp.pre_soft_cap_unit.token_unit = { { "my_account", 0 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.offer_unit = { { "bob", 2 }, { "$from.coining", 3 }, { "$from", 4 } };
      gp.pre_soft_cap_unit.token_unit = { { "alice", 5 }, { "$from", 3 } };
      op.initial_generation_policy = gp;
      op.validate();

      //FC_ASSERT( lower_bound > 0 )
      gp.min_offer_units_commitment.lower_bound = 0;
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( lower_bound >= SMT_MIN_HARD_CAP_OFFER_UNITS )
      gp.min_offer_units_commitment.lower_bound = SMT_MIN_HARD_CAP_OFFER_UNITS - 1;
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( upper_bound <= OFFER_MAX_SHARE_SUPPLY )
      gp.min_offer_units_commitment.upper_bound = OFFER_MAX_SHARE_SUPPLY + 1;
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( lower_bound <= upper_bound )
      gp.min_offer_units_commitment.lower_bound = OFFER_MAX_SHARE_SUPPLY - 1;
      gp.min_offer_units_commitment.upper_bound = gp.min_offer_units_commitment.lower_bound - 1;
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      gp.min_offer_units_commitment.lower_bound = 1 * SMT_MIN_HARD_CAP_OFFER_UNITS;
      gp.min_offer_units_commitment.upper_bound = 2 * SMT_MIN_HARD_CAP_OFFER_UNITS;
      gp.hard_cap_offer_units_commitment.lower_bound = 1 * SMT_MIN_HARD_CAP_OFFER_UNITS;
      gp.hard_cap_offer_units_commitment.upper_bound = 2 * SMT_MIN_HARD_CAP_OFFER_UNITS;
      op.initial_generation_policy = gp;
      gp.validate();
      ...
      //FC_ASSERT( soft_cap_percent <= OFFER_100_PERCENT )
      gp.soft_cap_percent = OFFER_100_PERCENT + 1;
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( soft_cap_percent == OFFER_100_PERCENT && post_soft_cap_unit.offer_unit.size() == 0 )
      gp.soft_cap_percent = OFFER_100_PERCENT;
      gp.post_soft_cap_unit.offer_unit = { { "bob", 2 } };
      gp.post_soft_cap_unit.token_unit = {};
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( soft_cap_percent == OFFER_100_PERCENT && post_soft_cap_unit.token_unit.size() == 0 )
      gp.soft_cap_percent = OFFER_100_PERCENT;
      gp.post_soft_cap_unit.offer_unit = {};
      gp.post_soft_cap_unit.token_unit = { { "alice", 3 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( soft_cap_percent != OFFER_100_PERCENT && post_soft_cap_unit.offer_unit.size() > 0 )
      gp.soft_cap_percent = OFFER_100_PERCENT / 2;
      gp.post_soft_cap_unit.offer_unit = {};
      gp.post_soft_cap_unit.token_unit = {};
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      //FC_ASSERT( min_offer_units_commitment.lower_bound <= hard_cap_offer_units_commitment.lower_bound )
      gp.min_offer_units_commitment.lower_bound = 10 * SMT_MIN_HARD_CAP_OFFER_UNITS;
      gp.min_offer_units_commitment.upper_bound = 20 * SMT_MIN_HARD_CAP_OFFER_UNITS;
      gp.hard_cap_offer_units_commitment.lower_bound = 9 * SMT_MIN_HARD_CAP_OFFER_UNITS;
      gp.hard_cap_offer_units_commitment.upper_bound = 20 * SMT_MIN_HARD_CAP_OFFER_UNITS;
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( min_offer_units_commitment.upper_bound <= hard_cap_offer_units_commitment.upper_bound )
      gp.hard_cap_offer_units_commitment.lower_bound = 11 * SMT_MIN_HARD_CAP_OFFER_UNITS;
      gp.hard_cap_offer_units_commitment.upper_bound = 19 * SMT_MIN_HARD_CAP_OFFER_UNITS;
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( hard_cap_offer_units_commitment.lower_bound >= SMT_MIN_SATURATION_OFFER_UNITS * uint64_t( max_unit_ratio ) )
      gp.hard_cap_offer_units_commitment.lower_bound = 11 * SMT_MIN_HARD_CAP_OFFER_UNITS;
      gp.hard_cap_offer_units_commitment.upper_bound = 21 * SMT_MIN_HARD_CAP_OFFER_UNITS;
      gp.max_unit_ratio = ( ( 11 * SMT_MIN_HARD_CAP_OFFER_UNITS ) / SMT_MIN_SATURATION_OFFER_UNITS ) * 2;
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      //FC_ASSERT( min_soft_cap >= SMT_MIN_SOFT_CAP_OFFER_UNITS )
      ...
      gp.post_soft_cap_unit.offer_unit = { { "abc", 1 } };
      gp.post_soft_cap_unit.token_unit = { { "abc1", max_val_16 } };
      gp.pre_soft_cap_unit.token_unit = { { "abc2", max_val_16 } };
      gp.min_offer_units_commitment.upper_bound = OFFER_MAX_SHARE_SUPPLY;
      gp.hard_cap_offer_units_commitment.upper_bound = OFFER_MAX_SHARE_SUPPLY;
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( max_offer_accepted <= max_share_supply_u128 )
      gp.min_unit_ratio = 1;
      gp.post_soft_cap_unit.token_unit = { { "abc1", 1 } };
      gp.pre_soft_cap_unit.token_unit = { { "abc2", 1 } };
      gp.post_soft_cap_unit.offer_unit = { { "abc3", max_val_16 } };
      gp.pre_soft_cap_unit.offer_unit = { { "abc34", max_val_16 } };
      op.initial_generation_policy = gp;
      OFFER_REQUIRE_THROW( op.validate(), fc::exception );
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( setup_apply )
{
   try
   {
      ...
      const offer::chain::smt_token_object* smt_token = db->find< offer::chain::smt_token_object, by_control_account >( op.control_account );
                                ...
                                fc::uint128_t nonce, database* db, smt_database_fixture& dbf )
{
   ...
   // Setup SMT
   {
      ...
      (
         ...
         dbf.get_cap_commitment( min_cap_val, nonce )/*min_offer_units_commitment*/,
         dbf.get_cap_commitment( max_cap_val, nonce )/*hard_cap_offer_units_commitment*/,
         OFFER_100_PERCENT/*soft_cap_percent*/,
         1/*min_unit_ratio*/,
         2/*max_unit_ratio*/
      );
   ...
   FC_ASSERT( smt_object.offer_units_min_cap < 0 && smt_object.offer_units_hard_cap < 0 );
   ...
   FC_ASSERT( smt_object.offer_units_min_cap < 0 && smt_object.offer_units_hard_cap == max_cap_val );
   ...
   PUSH_OP( op, private_key );
   FC_ASSERT( smt_object.offer_units_min_cap == min_cap_val && smt_object.offer_units_hard_cap == max_cap_val );
...
BOOST_AUTO_TEST_CASE( smt_cap_reveal_apply )
{
   try
   {
      ...
      setup_smt_and_reveal_caps("alice", alice_private_key, smts[0], 1, SMT_MIN_HARD_CAP_OFFER_UNITS + 1, 20000, 0, db, *this);
      // Test hidden caps (1234 nonce).
      setup_smt_and_reveal_caps("alice", alice_private_key, smts[1], 10000, SMT_MIN_HARD_CAP_OFFER_UNITS + 1, 20000, 1234, db, *this);
   }
   FC_LOG_AND_RETHROW()
}
