...
function print_help_and_quit {
   ...
   echo "Example: 10000 --pre-appbase https://offerd.offerit.org http://127.0.0.1:6990"
   exit $EXIT_CODE
}
...
[ $? -ne 0 ] && echo FATAL: offerd not running at $NODE1? && exit -1

echo Checking $NODE2...
pyresttest $NODE2 ./basic_smoketest.yaml
[ $? -ne 0 ] && echo FATAL: offerd not running at $NODE2? && exit -1
