...
{
   static_assert( OFFER_ASSET_SYMBOL_NAI_STRING_LENGTH >= 12, "This code will overflow a short buffer" );
   uint32_t x = to_nai();
   buf[11] = '\0';
   buf[10] = ((x%10)+'0');  x /= 10;
   buf[ 9] = ((x%10)+'0');  x /= 10;
...
uint32_t asset_symbol_type::asset_num_from_nai( uint32_t nai, uint8_t decimal_places )
{
   ...
   {
      case OFFER_NAI_OFFER:
         FC_ASSERT( decimal_places == OFFER_PRECISION_OFFER );
         return OFFER_ASSET_NUM_OFFER;
      case OFFER_NAI_BSD:
         FC_ASSERT( decimal_places == OFFER_PRECISION_BSD );
         return OFFER_ASSET_NUM_BSD;
      case OFFER_NAI_COINS:
         FC_ASSERT( decimal_places == OFFER_PRECISION_COINS );
         return OFFER_ASSET_NUM_COINS;
      default:
         FC_ASSERT( decimal_places <= OFFER_ASSET_MAX_DECIMALS, "Invalid decimal_places" );
...
uint32_t asset_symbol_type::to_nai()const
{
   ...
   switch( asset_num )
   {
      case OFFER_ASSET_NUM_OFFER:
         nai_data_digits = OFFER_NAI_OFFER;
         break;
      case OFFER_ASSET_NUM_BSD:
         nai_data_digits = OFFER_NAI_BSD;
         break;
      case OFFER_ASSET_NUM_COINS:
         nai_data_digits = OFFER_NAI_COINS;
         break;
      default:
...
asset_symbol_type asset_symbol_type::get_paired_symbol() const
{
   switch( space() )
   {
      case legacy_space:
      {
         ...
         {
            case OFFER_ASSET_NUM_OFFER:
               return from_asset_num( OFFER_ASSET_NUM_COINS );
            case OFFER_ASSET_NUM_BSD:
               return *this;
            case OFFER_ASSET_NUM_COINS:
               return from_asset_num( OFFER_ASSET_NUM_OFFER );
            default:
               FC_ASSERT( false, "Unknown asset symbol" );
         }
      }
...
void asset::validate()const
{
   ...
   FC_ASSERT( amount.value <= OFFER_MAX_SATOSHIS );
}
...
DEFINE_PRICE_COMPARISON_OPERATOR( >= )
      ...
      price price::max( asset_symbol_type base, asset_symbol_type quote ) { return asset( share_type(OFFER_MAX_SATOSHIS), base ) / asset( share_type(1), quote); }
      price price::min( asset_symbol_type base, asset_symbol_type quote ) { return asset( 1, base ) / asset( OFFER_MAX_SATOSHIS, quote); }
...
namespace fc {
   void to_variant( const offer::protocol::asset& var, fc::variant& vo )
   {
      try
      {
   ...
   void from_variant( const fc::variant& var, offer::protocol::asset& vo )
   {
      try
      {
         ...
         vo.symbol = offer::protocol::asset_symbol_type::from_nai_string( v_object[ "nai" ].as< std::string >().c_str(), v_object[ "precision" ].as< uint8_t >() );
      } FC_CAPTURE_AND_RETHROW()
   }
}
