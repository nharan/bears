...
void smt_create_operation::validate()const
{
   ...
   FC_ASSERT( smt_creation_fee.amount <= OFFER_MAX_SHARE_SUPPLY, "Fee must be smaller than OFFER_MAX_SHARE_SUPPLY" );
   FC_ASSERT( is_asset_type( smt_creation_fee, OFFER_SYMBOL ) || is_asset_type( smt_creation_fee, BSD_SYMBOL ), "Fee must be OFFER or BSD" );
...
uint32_t smt_generation_unit::offer_unit_sum()const
{
   uint32_t result = 0;
   for(const std::pair< account_name_type, uint16_t >& e : offer_unit )
      result += e.second;
   return result;
}
...
void smt_capped_generation_policy::validate()const
{
   ...
   FC_ASSERT( pre_soft_cap_unit.offer_unit.size() > 0 );
   FC_ASSERT( pre_soft_cap_unit.token_unit.size() > 0 );

   FC_ASSERT( pre_soft_cap_unit.offer_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( pre_soft_cap_unit.token_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( post_soft_cap_unit.offer_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( post_soft_cap_unit.token_unit.size() <= SMT_MAX_UNIT_COUNT );

   // TODO : Check account name

   if( soft_cap_percent == OFFER_100_PERCENT )
   {
      FC_ASSERT( post_soft_cap_unit.offer_unit.size() == 0 );
   ...
   min_offer_units_commitment.validate();
   hard_cap_offer_units_commitment.validate();
   ...
   FC_ASSERT( hard_cap_offer_units_commitment.lower_bound >= SMT_MIN_HARD_CAP_OFFER_UNITS );
   ...
   static_assert( uint64_t( OFFER_MAX_SHARE_SUPPLY ) < (std::numeric_limits< uint64_t >::max() / OFFER_100_PERCENT), "Overflow check failed" );
   uint64_t min_soft_cap = (uint64_t( hard_cap_offer_units_commitment.lower_bound.value ) * soft_cap_percent) / OFFER_100_PERCENT;
   uint64_t max_soft_cap = (uint64_t( hard_cap_offer_units_commitment.upper_bound.value ) * soft_cap_percent) / OFFER_100_PERCENT;

   // we want soft cap to be large enough we don't see quantization effects
   FC_ASSERT( min_soft_cap >= SMT_MIN_SOFT_CAP_OFFER_UNITS );

   // We want to prevent the following from overflowing OFFER_MAX_SHARE_SUPPLY:
   // max_tokens_created = (u1.tt * sc + u2.tt * (hc-sc)) * min_unit_ratio
   // max_offer_accepted =  u1.st * sc + u2.st * (hc-sc)

   // hc / max_unit_ratio is the saturation point

   uint128_t sc = max_soft_cap;
   uint128_t hc_sc = hard_cap_offer_units_commitment.upper_bound.value - max_soft_cap;

   uint128_t max_tokens_created = (pre_soft_cap_unit.token_unit_sum() * sc + post_soft_cap_unit.token_unit_sum() * hc_sc) * min_unit_ratio;
   uint128_t max_share_supply_u128 = uint128_t( OFFER_MAX_SHARE_SUPPLY );

   FC_ASSERT( max_tokens_created <= max_share_supply_u128 );

   uint128_t max_offer_accepted = (pre_soft_cap_unit.offer_unit_sum() * sc + post_soft_cap_unit.offer_unit_sum() * hc_sc);
...
void smt_setup_emissions_operation::validate()const
{
   ...
   FC_ASSERT( schedule_time > OFFER_GENESIS_TIME );
