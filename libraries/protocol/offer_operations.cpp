...
namespace offer { namespace protocol {
   ...
   void validate_auth_size( const authority& a )
   {
      ...
      FC_ASSERT( size <= OFFER_MAX_AUTHORITY_MEMBERSHIP, "Authority membership exceeded. Max: 10 Current: ${n}", ("n", size) );
   }

   void account_create_operation::validate() const
   {
      validate_account_name( new_account_name );
      FC_ASSERT( is_asset_type( fee, OFFER_SYMBOL ), "Account creation fee must be OFFER" );
      owner.validate();
      active.validate();
      ...
      FC_ASSERT( fee >= asset( 0, OFFER_SYMBOL ), "Account creation fee cannot be negative" );
   }
   ...
   void comment_payout_beneficiaries::validate()const
   {
      ...
      FC_ASSERT( beneficiaries[0].weight <= OFFER_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
      sum += beneficiaries[0].weight;
      FC_ASSERT( sum <= OFFER_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

      for( size_t i = 1; i < beneficiaries.size(); i++ )
      {
         validate_account_name( beneficiaries[i].account );
         FC_ASSERT( beneficiaries[i].weight <= OFFER_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
   ...
   {
      validate_account_name( author );
      FC_ASSERT( percent_offer_dollars <= OFFER_100_PERCENT, "Percent cannot exceed 100%" );
   ...
   void claim_account_operation::validate()const
   {
      ...
      FC_ASSERT( is_asset_type( fee, OFFER_SYMBOL ), "Account creation fee must be OFFER" );
      FC_ASSERT( fee >= asset( 0, OFFER_SYMBOL ), "Account creation fee cannot be negative" );
      FC_ASSERT( fee <= asset( OFFER_MAX_ACCOUNT_CREATION_FEE, OFFER_SYMBOL ), "Account creation fee cannot be too large" );
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= OFFER_100_PERCENT, "Weight is not a OFFERHARES percentage" );
      validate_permlink( permlink );
   }
   ...
   { try {
      ...
      FC_ASSERT( amount.symbol != COINS_SYMBOL, "transferring of Offer Power (SHRP) is not allowed." );
      FC_ASSERT( amount.amount > 0, "Cannot transfer a negative amount (aka: stealing)" );
      FC_ASSERT( memo.size() < OFFER_MAX_MEMO_SIZE, "Memo is too large" );
   ...
   void set_withdraw_coining_route_operation::validate() const
   {
      ...
      FC_ASSERT( 0 <= percent && percent <= OFFER_100_PERCENT, "Percent must be valid offer percent" );
   }
   ...
   void witness_update_operation::validate() const
   {
      ...
      FC_ASSERT( url.size() <= OFFER_MAX_WITNESS_URL_LENGTH, "URL is too long" );

      FC_ASSERT( url.size() > 0, "URL size must be greater than 0" );
      FC_ASSERT( fc::is_utf8( url ), "URL is not valid UTF8" );
      FC_ASSERT( fee >= asset( 0, OFFER_SYMBOL ), "Fee cannot be negative" );
   ...
   void witness_set_properties_operation::validate() const
   {
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_creation_fee.symbol == OFFER_SYMBOL, "account_creation_fee must be in OFFER" );
         FC_ASSERT( account_creation_fee.amount >= OFFER_MIN_ACCOUNT_CREATION_FEE, "account_creation_fee smaller than minimum account creation fee" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( maximum_block_size >= OFFER_MIN_BLOCK_SIZE_LIMIT, "maximum_block_size smaller than minimum max block size" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( bsd_interest_rate <= OFFER_100_PERCENT, "bsd_interest_rate must not exceed 100%" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( ( is_asset_type( bsd_exchange_rate.base, BSD_SYMBOL ) && is_asset_type( bsd_exchange_rate.quote, OFFER_SYMBOL ) ),
            "Price feed must be a OFFER/BSD price" );
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_budget >= OFFER_RD_MIN_BUDGET, "Budget must be at least ${n}", ("n", OFFER_RD_MIN_BUDGET) );
         FC_ASSERT( account_subsidy_budget <= OFFER_RD_MAX_BUDGET, "Budget must be at most ${n}", ("n", OFFER_RD_MAX_BUDGET) );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_decay >= OFFER_RD_MIN_DECAY, "Decay must be at least ${n}", ("n", OFFER_RD_MIN_DECAY) );
         FC_ASSERT( account_subsidy_decay <= OFFER_RD_MAX_DECAY, "Decay must be at most ${n}", ("n", OFFER_RD_MAX_DECAY) );
      }
   }
   ...
   void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
   {
      ...
      proof = fc::equihash::proof::hash( OFFER_EQUIHASH_N, OFFER_EQUIHASH_K, seed );
   ...
   void equihash_pow::validate() const
   {
      validate_account_name( input.worker_account );
      auto seed = fc::sha256::hash( input );
      FC_ASSERT( proof.n == OFFER_EQUIHASH_N, "proof of work 'n' value is incorrect" );
      FC_ASSERT( proof.k == OFFER_EQUIHASH_K, "proof of work 'k' value is incorrect" );
   ...
   void feed_publish_operation::validate()const
   {
      ...
      FC_ASSERT( ( is_asset_type( exchange_rate.base, OFFER_SYMBOL ) && is_asset_type( exchange_rate.quote, BSD_SYMBOL ) )
         || ( is_asset_type( exchange_rate.base, BSD_SYMBOL ) && is_asset_type( exchange_rate.quote, OFFER_SYMBOL ) ),
         "Price feed must be a OFFER/BSD price" );
   ...
   void limit_order_create_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, OFFER_SYMBOL ) && is_asset_type( min_to_receive, BSD_SYMBOL ) )
               || ( is_asset_type( amount_to_sell, BSD_SYMBOL ) && is_asset_type( min_to_receive, OFFER_SYMBOL ) )
               || (
                     amount_to_sell.symbol.space() == asset_symbol_type::smt_nai_space
                     && is_asset_type( min_to_receive, OFFER_SYMBOL )
                  )
               || (
                     is_asset_type( amount_to_sell, OFFER_SYMBOL )
                     && min_to_receive.symbol.space() == asset_symbol_type::smt_nai_space
                  ),
               "Limit order must be for the OFFER:BSD or SMT:(OFFER/BSD) market" );
   ...
   void limit_order_create2_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, OFFER_SYMBOL ) && is_asset_type( exchange_rate.quote, BSD_SYMBOL ) )
               || ( is_asset_type( amount_to_sell, BSD_SYMBOL ) && is_asset_type( exchange_rate.quote, OFFER_SYMBOL ) )
               || (
                     amount_to_sell.symbol.space() == asset_symbol_type::smt_nai_space
                     && is_asset_type( exchange_rate.quote, OFFER_SYMBOL )
                  )
               || (
                     is_asset_type( amount_to_sell, OFFER_SYMBOL )
   ...
   {
      validate_account_name( owner );
      /// only allow conversion from BSD to OFFER, allowing the opposite can enable traders to abuse
      /// market fluxuations through converting large quantities without moving the price.
      FC_ASSERT( is_asset_type( amount, BSD_SYMBOL ), "Can only convert BSD to OFFER" );
   ...
   void escrow_transfer_operation::validate()const
   {
      ...
      FC_ASSERT( offer_amount.amount >= 0, "offer amount cannot be negative" );
      FC_ASSERT( bsd_amount.amount > 0 || offer_amount.amount > 0, "escrow must transfer a non-zero amount" );
      FC_ASSERT( from != agent && to != agent, "agent must be a third party" );
      FC_ASSERT( (fee.symbol == OFFER_SYMBOL) || (fee.symbol == BSD_SYMBOL), "fee must be OFFER or BSD" );
      FC_ASSERT( bsd_amount.symbol == BSD_SYMBOL, "bsd amount must contain BSD" );
      FC_ASSERT( offer_amount.symbol == OFFER_SYMBOL, "offer amount must contain OFFER" );
   ...
   void escrow_release_operation::validate()const
   {
      ...
      FC_ASSERT( bsd_amount.amount >= 0, "bsd amount cannot be negative" );
      FC_ASSERT( offer_amount.amount >= 0, "offer amount cannot be negative" );
      FC_ASSERT( bsd_amount.amount > 0 || offer_amount.amount > 0, "escrow must release a non-zero amount" );
      FC_ASSERT( bsd_amount.symbol == BSD_SYMBOL, "bsd amount must contain BSD" );
      FC_ASSERT( offer_amount.symbol == OFFER_SYMBOL, "offer amount must contain OFFER" );
   }
   ...
   void transfer_to_savings_operation::validate()const {
      ...
      FC_ASSERT( amount.symbol == OFFER_SYMBOL || amount.symbol == BSD_SYMBOL );
      FC_ASSERT( memo.size() < OFFER_MAX_MEMO_SIZE, "Memo is too large" );
      FC_ASSERT( fc::is_utf8( memo ), "Memo is not UTF8" );
   }
   ...
   {
      validate_account_name( account );
      FC_ASSERT( is_asset_type( reward_offer, OFFER_SYMBOL ), "Reward Offer must be OFFER" );
      FC_ASSERT( is_asset_type( reward_bsd, BSD_SYMBOL ), "Reward Offer must be BSD" );
      FC_ASSERT( is_asset_type( reward_coins, COINS_SYMBOL ), "Reward Offer must be COINS" );
      FC_ASSERT( reward_offer.amount >= 0, "Cannot claim a negative amount" );
      ...
      FC_ASSERT( reward_offer.amount > 0 || reward_bsd.amount > 0 || reward_coins.amount > 0, "Must claim something." );
   }
