...
{
   FC_ASSERT( _db.has_hardfork( OFFER_SMT_HARDFORK ), "SMT functionality not enabled until hardfork ${hf}", ("hf", OFFER_SMT_HARDFORK) );
   ...
   FC_ASSERT( dgpo.smt_creation_fee.symbol == OFFER_SYMBOL || dgpo.smt_creation_fee.symbol == BSD_SYMBOL,
      "Unexpected internal error - wrong symbol ${s} of SMT creation fee.", ("s", dgpo.smt_creation_fee.symbol) );
   FC_ASSERT( o.smt_creation_fee.symbol == OFFER_SYMBOL || o.smt_creation_fee.symbol == BSD_SYMBOL,
      "Asset fee must be OFFER or BSD, was ${s}", ("s", o.smt_creation_fee.symbol) );
   ...
   else
   {
      ...
      if( o.smt_creation_fee.symbol == OFFER_SYMBOL )
      {
         effective_elevation_fee = _db.to_bsd( o.smt_creation_fee );
      }
      else
      {
         effective_elevation_fee = _db.to_offer( o.smt_creation_fee );         
      }
   }
   ...
   _db.adjust_balance( OFFER_NULL_ACCOUNT,  o.smt_creation_fee );
...
void smt_cap_reveal_evaluator::do_apply( const smt_cap_reveal_operation& o )
{
   ...
   FC_ASSERT( smt.offer_units_min_cap < 0 || smt.offer_units_hard_cap < 0, "Both min cap and max hard cap have already been revealed" );

   if( smt.offer_units_min_cap < 0 )
      try
      {
         o.cap.validate( smt.capped_generation_policy.min_offer_units_commitment );
         _db.modify( smt, [&]( smt_token_object& smt_object )
         {
            smt_object.offer_units_min_cap = o.cap.amount;
         });
         return;
      }
      catch( const fc::exception& e )
      {
         if( smt.offer_units_hard_cap >= 0 )
            throw;
      }

   o.cap.validate( smt.capped_generation_policy.hard_cap_offer_units_commitment );
   _db.modify( smt, [&]( smt_token_object& smt_object )
   {
      smt_object.offer_units_hard_cap = o.cap.amount;
   });
}
