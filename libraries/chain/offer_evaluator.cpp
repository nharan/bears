...
{
   FC_ASSERT( permlink.size() > OFFER_MIN_PERMLINK_LENGTH && permlink.size() < OFFER_MAX_PERMLINK_LENGTH, "Permlink is not a valid size." );
...
void witness_update_evaluator::do_apply( const witness_update_operation& o )
{
   ...
   if ( _db.has_hardfork( OFFER_HARDFORK_0_14__410 ) )
   {
      FC_ASSERT( o.props.account_creation_fee.symbol.is_canon() );
      if( _db.has_hardfork( OFFER_HARDFORK_0_20__2651 ) || _db.is_producing() )
      {
         FC_TODO( "Move to validate() after HF20" );
         FC_ASSERT( o.props.account_creation_fee.amount <= OFFER_MAX_ACCOUNT_CREATION_FEE, "account_creation_fee greater than maximum account creation fee" );
      }
   }
   ...
   if( _db.is_producing() || _db.has_hardfork( OFFER_HARDFORK_0_20__2642 ))
   {
      FC_ASSERT( o.props.maximum_block_size <= OFFER_SOFT_MAX_BLOCK_SIZE, "Max block size cannot be more than 2MiB" );
   }
...
{
   FC_ASSERT( _db.has_hardfork( OFFER_HARDFORK_0_20__1620 ), "witness_set_properties_evaluator not enabled until HF 20" );
   ...
   if( flags.account_creation_changed )
   {
      ...
      {
         FC_TODO( "Move to validate() after HF20" );
         FC_ASSERT( props.account_creation_fee.amount <= OFFER_MAX_ACCOUNT_CREATION_FEE, "account_creation_fee greater than maximum account creation fee" );
      }
   }
   ...
   const dynamic_global_property_object& props, bool mined, const account_name_type& recovery_account, uint32_t hardfork )
{
   ...
   if( hardfork < OFFER_HARDFORK_0_20__2539 )
   {
      acc.voting_manabar.current_mana = OFFER_100_PERCENT;
   }

   if( hardfork >= OFFER_HARDFORK_0_11 )
   {
      FC_TODO( "If after HF 20, there are no temp account creations, the HF check can be removed." )
      if( ( hardfork < OFFER_HARDFORK_0_20__1782 ) || ( recovery_account != OFFER_TEMP_ACCOUNT ) )
      {
...
void account_create_evaluator::do_apply( const account_create_operation& o )
{
   ...
   {
      FC_TODO( "Move to validate() after HF20" );
      FC_ASSERT( o.fee <= asset( OFFER_MAX_ACCOUNT_CREATION_FEE, OFFER_SYMBOL ), "Account creation fee cannot be too large" );
   }

   if( _db.has_hardfork( OFFER_HARDFORK_0_20__1771 ) )
   {
      FC_ASSERT( o.fee == wso.median_props.account_creation_fee, "Must pay the exact account creation fee. paid: ${p} fee: ${f}",
                  ("p", o.fee)
                  ("f", wso.median_props.account_creation_fee) );
   }
   else if( !_db.has_hardfork( OFFER_HARDFORK_0_20__1761 ) && _db.has_hardfork( OFFER_HARDFORK_0_19__987 ) )
   {
      FC_ASSERT( o.fee >= asset( wso.median_props.account_creation_fee.amount * OFFER_CREATE_ACCOUNT_WITH_OFFER_MODIFIER, OFFER_SYMBOL ), "Insufficient Fee: ${f} required, ${p} provided.",
                 ("f", wso.median_props.account_creation_fee * asset( OFFER_CREATE_ACCOUNT_WITH_OFFER_MODIFIER, OFFER_SYMBOL ) )
                 ("p", o.fee) );
   }
   else if( _db.has_hardfork( OFFER_HARDFORK_0_1 ) )
   {
   ...
   if( _db.is_producing() || _db.has_hardfork( OFFER_HARDFORK_0_20 ) )
   {
   ...
   if( _db.has_hardfork( OFFER_HARDFORK_0_15__465 ) )
   {
   ...
   if( _db.has_hardfork( OFFER_HARDFORK_0_20__1762 ) )
   {
      _db.adjust_balance( _db.get< account_object, by_name >( OFFER_NULL_ACCOUNT ), o.fee );
   }
   ...
   if( !_db.has_hardfork( OFFER_HARDFORK_0_20__1762 ) && o.fee.amount > 0 )
   {
...
{
   FC_ASSERT( !_db.has_hardfork( OFFER_HARDFORK_0_20__1760 ), "Account creation with delegation is deprecated as of Hardfork 20" );
   ...
   auto target_delegation = asset( wso.median_props.account_creation_fee.amount * OFFER_CREATE_ACCOUNT_WITH_OFFER_MODIFIER * OFFER_CREATE_ACCOUNT_DELEGATION_RATIO, OFFER_SYMBOL ) * props.get_coining_share_price();

   auto current_delegation = asset( o.fee.amount * OFFER_CREATE_ACCOUNT_DELEGATION_RATIO, OFFER_SYMBOL ) * props.get_coining_share_price() + o.delegation;
   ...
   if( o.delegation.amount > 0 || !_db.has_hardfork( OFFER_HARDFORK_0_19__997 ) )
   {
      _db.create< coining_delegation_object >( [&]( coining_delegation_object& vdo )
      {
         ...
         vdo.min_delegation_time = _db.head_block_time() + OFFER_CREATE_ACCOUNT_DELEGATION_TIME;
      });
   }
...
{
   if( _db.has_hardfork( OFFER_HARDFORK_0_1 ) ) FC_ASSERT( o.account != OFFER_TEMP_ACCOUNT, "Cannot update temp account." );

   if( ( _db.has_hardfork( OFFER_HARDFORK_0_15__465 ) ) && o.posting )
      o.posting->validate();
   ...
   {
#ifndef IS_TEST_NET
      if( _db.has_hardfork( OFFER_HARDFORK_0_11 ) )
         FC_ASSERT( _db.head_block_time() - account_auth.last_owner_update > OFFER_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
#endif
   ...
   if( o.active && ( _db.has_hardfork( OFFER_HARDFORK_0_15__465 ) ) )
      verify_authority_accounts_exist( _db, *o.active, o.account, authority::active );
   if( o.posting && ( _db.has_hardfork( OFFER_HARDFORK_0_15__465 ) ) )
...
void delete_comment_evaluator::do_apply( const delete_comment_operation& o )
{
   ...
   if( _db.has_hardfork( OFFER_HARDFORK_0_19__876 ) )
      FC_ASSERT( comment.cashout_time != fc::time_point_sec::maximum() );

   if( _db.has_hardfork( OFFER_HARDFORK_0_19__977 ) )
   ...
   if( _db.has_hardfork( OFFER_HARDFORK_0_6__80 ) && comment.parent_author != OFFER_ROOT_POST_PARENT )
   {
...
void comment_options_evaluator::do_apply( const comment_options_operation& o )
{
   ...
   FC_ASSERT( comment.percent_offer_dollars >= o.percent_offer_dollars, "A comment cannot accept a greater percent BSD." );

   _db.modify( comment, [&]( comment_object& c ) {
       c.max_accepted_payout   = o.max_accepted_payout;
       c.percent_offer_dollars = o.percent_offer_dollars;
...
{ try {
   if( _db.has_hardfork( OFFER_HARDFORK_0_5__55 ) )
   ...
   if( o.parent_author != OFFER_ROOT_POST_PARENT )
   {
      parent = &_db.get_comment( o.parent_author, o.parent_permlink );
      if( !_db.has_hardfork( OFFER_HARDFORK_0_17__767 ) )
         FC_ASSERT( parent->depth < OFFER_MAX_COMMENT_DEPTH_PRE_HF17, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",OFFER_MAX_COMMENT_DEPTH_PRE_HF17) );
      else
         FC_ASSERT( parent->depth < OFFER_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",OFFER_MAX_COMMENT_DEPTH) );
   }
   ...
   if ( itr == by_permlink_idx.end() )
   {
      if( o.parent_author != OFFER_ROOT_POST_PARENT )
      {
         ...
         if( _db.has_hardfork( OFFER_HARDFORK_0_12__177 ) && !_db.has_hardfork( OFFER_HARDFORK_0_17__869 ) )
      ...
      if( _db.has_hardfork( OFFER_HARDFORK_0_20__2019 ) )
      {
         if( o.parent_author == OFFER_ROOT_POST_PARENT )
             FC_ASSERT( ( now - auth.last_root_post ) > OFFER_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
         else
             FC_ASSERT( (now - auth.last_post) >= OFFER_MIN_REPLY_INTERVAL_HF20, "You may only comment once every 3 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else if( _db.has_hardfork( OFFER_HARDFORK_0_12__176 ) )
      {
         if( o.parent_author == OFFER_ROOT_POST_PARENT )
             FC_ASSERT( ( now - auth.last_root_post ) > OFFER_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
         else
             FC_ASSERT( (now - auth.last_post) > OFFER_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else if( _db.has_hardfork( OFFER_HARDFORK_0_6__113 ) )
      {
         if( o.parent_author == OFFER_ROOT_POST_PARENT )
             FC_ASSERT( (now - auth.last_post) > OFFER_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("auth.last_post",auth.last_post) );
         else
             FC_ASSERT( (now - auth.last_post) > OFFER_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else
      ...
      uint16_t reward_weight = OFFER_100_PERCENT;
      uint64_t post_bandwidth = auth.post_bandwidth;

      if( _db.has_hardfork( OFFER_HARDFORK_0_12__176 ) && !_db.has_hardfork( OFFER_HARDFORK_0_17__733 ) && o.parent_author == OFFER_ROOT_POST_PARENT )
      {
         uint64_t post_delta_time = std::min( _db.head_block_time().sec_since_epoch() - auth.last_root_post.sec_since_epoch(), OFFER_POST_AVERAGE_WINDOW );
         uint32_t old_weight = uint32_t( ( post_bandwidth * ( OFFER_POST_AVERAGE_WINDOW - post_delta_time ) ) / OFFER_POST_AVERAGE_WINDOW );
         post_bandwidth = ( old_weight + OFFER_100_PERCENT );
         reward_weight = uint16_t( std::min( ( OFFER_POST_WEIGHT_CONSTANT * OFFER_100_PERCENT ) / ( post_bandwidth * post_bandwidth ), uint64_t( OFFER_100_PERCENT ) ) );
      }
      ...
      {
         if( _db.has_hardfork( OFFER_HARDFORK_0_1 ) )
         {
         ...
         if ( o.parent_author == OFFER_ROOT_POST_PARENT )
         {
            ...
            com.cashout_time = _db.has_hardfork( OFFER_HARDFORK_0_12__177 ) ?
               _db.head_block_time() + OFFER_CASHOUT_WINDOW_SECONDS_PRE_HF17 :
         ...
         if( _db.has_hardfork( OFFER_HARDFORK_0_17__769 ) )
         {
            com.cashout_time = com.created + OFFER_CASHOUT_WINDOW_SECONDS;
         }
      });
...
/// this loop can be skiped for validate-only nodes as it is merely gathering stats for indicies
   ...
   else // start edit case
   {
      ...
      if( !_db.has_hardfork( OFFER_HARDFORK_0_17__772 ) )
      {
         if( _db.has_hardfork( OFFER_HARDFORK_0_14__306 ) )
            FC_ASSERT( _db.calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum(), "The comment is archived." );
         else if( _db.has_hardfork( OFFER_HARDFORK_0_10 ) )
...
void escrow_transfer_evaluator::do_apply( const escrow_transfer_operation& o )
{
   try
   {
      ...
      asset offer_spent = o.offer_amount;
      asset bsd_spent = o.bsd_amount;
      if( o.fee.symbol == OFFER_SYMBOL )
         offer_spent += o.fee;
      else
         bsd_spent += o.fee;

      FC_ASSERT( from_account.balance >= offer_spent, "Account cannot cover OFFER costs of escrow. Required: ${r} Available: ${a}", ("r",offer_spent)("a",from_account.balance) );
      ...
      _db.adjust_balance( from_account, -offer_spent );
      ...
      _db.create<escrow_object>([&]( escrow_object& esc )
      {
         ...
         esc.offer_balance          = o.offer_amount;
...
void escrow_approve_evaluator::do_apply( const escrow_approve_operation& o )
{
   try
   {
      ...
      if( reject_escrow )
      {
         _db.adjust_balance( o.from, escrow.offer_balance );
...
void escrow_release_evaluator::do_apply( const escrow_release_operation& o )
{
   try
   {
      ...
      const auto& e = _db.get_escrow( o.from, o.escrow_id );
      FC_ASSERT( e.offer_balance >= o.offer_amount, "Release amount exceeds escrow balance. Amount: ${a}, Balance: ${b}", ("a", o.offer_amount)("b", e.offer_balance) );
      ...
      _db.adjust_balance( o.receiver, o.offer_amount );
      ...
      if( e.offer_balance.amount == 0 && e.bsd_balance.amount == 0 )
      {
         _db.remove( e );
      }
   }
...
void withdraw_coining_evaluator::do_apply( const withdraw_coining_operation& o )
{
   ...
   FC_ASSERT( account.coining_shares >= asset( 0, COINS_SYMBOL ), "Account does not have sufficient Offer Power for withdraw." );
   FC_ASSERT( account.coining_shares - account.delegated_coining_shares >= o.coining_shares, "Account does not have sufficient Offer Power for withdraw." );

   FC_TODO( "Remove this entire block after HF 20" )
   if( !_db.has_hardfork( OFFER_HARDFORK_0_20__1860 ) && !account.mined && _db.has_hardfork( OFFER_HARDFORK_0_1 ) )
   {
      const auto& props = _db.get_dynamic_global_properties();
      const witness_schedule_object& wso = _db.get_witness_schedule_object();
      ...
      FC_ASSERT( account.coining_shares > min_coins || ( _db.has_hardfork( OFFER_HARDFORK_0_16__562 ) && o.coining_shares.amount == 0 ),
                 "Account registered by another account requires 10x account creation fee worth of Offer Power before it can be powered down." );
   }

   if( o.coining_shares.amount == 0 )
   {
      if( _db.has_hardfork( OFFER_HARDFORK_0_5__57 ) )
   ...
   {
      int coining_withdraw_intervals = OFFER_COINING_WITHDRAW_INTERVALS_PRE_HF_16;
      if( _db.has_hardfork( OFFER_HARDFORK_0_16__551 ) )
         coining_withdraw_intervals = OFFER_COINING_WITHDRAW_INTERVALS; /// 13 weeks = 1 quarter of a year
      ...
      _db.modify( account, [&]( account_object& a )
      {
         ...
         a.next_coining_withdrawal = _db.head_block_time() + fc::seconds(OFFER_COINING_WITHDRAW_INTERVAL_SECONDS);
...
void set_withdraw_coining_route_evaluator::do_apply( const set_withdraw_coining_route_operation& o )
{
   ...
   if( itr == wd_idx.end() )
   {
      ...
      FC_ASSERT( from_account.withdraw_routes < OFFER_MAX_WITHDRAW_ROUTES, "Account already has the maximum number of routes." );
   ...
   FC_ASSERT( total_percent <= OFFER_100_PERCENT, "More than 100% of coining withdrawals allocated to destinations." );
   }
...
void account_witness_proxy_evaluator::do_apply( const account_witness_proxy_operation& o )
{
   ...
   if( o.proxy.size() ) {
      ...
      proxy_chain.reserve( OFFER_MAX_PROXY_RECURSION_DEPTH + 1 );
      ...
      while( cprox->proxy.size() != 0 ) {
         ...
         cprox = &next_proxy;
         FC_ASSERT( proxy_chain.size() <= OFFER_MAX_PROXY_RECURSION_DEPTH, "Proxy chain is too long." );
      }
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
   ...
   if( itr == by_account_witness_idx.end() ) {
      ...
      if ( _db.has_hardfork( OFFER_HARDFORK_0_2 ) )
      {
         FC_ASSERT( voter.witnesses_voted_for < OFFER_MAX_ACCOUNT_WITNESS_VOTES, "Account has voted for too many witnesses." ); // TODO: Remove after hardfork 2
         ...
         if( _db.has_hardfork( OFFER_HARDFORK_0_3 ) ) {
...
void pre_hf20_vote_evaluator( const vote_operation& o, database& _db )
{
   ...
   if( _db.has_hardfork( OFFER_HARDFORK_0_12__177 ) && _db.calculate_discussion_payout_time( comment ) == fc::time_point_sec::maximum() )
   {
#ifndef CLEAR_VOTES
   ...
   if( _db.has_hardfork( OFFER_HARDFORK_0_11 ) )
      FC_ASSERT( elapsed_seconds >= OFFER_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );

   int64_t regenerated_power = (OFFER_100_PERCENT * elapsed_seconds) / OFFER_VOTING_MANA_REGENERATION_SECONDS;
   int64_t current_power     = std::min( int64_t(voter.voting_manabar.current_mana) + regenerated_power, int64_t(OFFER_100_PERCENT) );
   ...
   int64_t  used_power  = ((current_power * abs_weight) / OFFER_100_PERCENT) * (60*60*24);

   const dynamic_global_property_object& dgpo = _db.get_dynamic_global_properties();

   // The second multiplication is rounded up as of HF 259
   int64_t max_vote_denom = dgpo.vote_power_reserve_rate * OFFER_VOTING_MANA_REGENERATION_SECONDS;
   FC_ASSERT( max_vote_denom > 0 );

   if( !_db.has_hardfork( OFFER_HARDFORK_0_14__259 ) )
   {
   ...
   int64_t abs_rshares    = ((uint128_t( _db.get_effective_coining_shares( voter, COINS_SYMBOL ).amount.value ) * used_power) / (OFFER_100_PERCENT)).to_uint64();
   if( !_db.has_hardfork( OFFER_HARDFORK_0_14__259 ) && abs_rshares == 0 ) abs_rshares = 1;

   if( _db.has_hardfork( OFFER_HARDFORK_0_20__1764 ) )
   {
      abs_rshares -= OFFER_VOTE_DUST_THRESHOLD;
      abs_rshares = std::max( int64_t(0), abs_rshares );
   }
   else if( _db.has_hardfork( OFFER_HARDFORK_0_14__259 ) )
   {
      FC_ASSERT( abs_rshares > OFFER_VOTE_DUST_THRESHOLD || o.weight == 0, "Voting weight is too small, please accumulate more voting power or offer power." );
   }
   else if( _db.has_hardfork( OFFER_HARDFORK_0_13__248 ) )
   {
      FC_ASSERT( abs_rshares > OFFER_VOTE_DUST_THRESHOLD || abs_rshares == 1, "Voting weight is too small, please accumulate more voting power or offer power." );
   }
   ...
   {
      if( _db.has_hardfork( OFFER_HARDFORK_0_12__177 ) )
   ...
   if( itr == comment_vote_idx.end() )
   {
      ...
      if( rshares > 0 )
      {
         if( _db.has_hardfork( OFFER_HARDFORK_0_17__900 ) )
            FC_ASSERT( _db.head_block_time() < comment.cashout_time - OFFER_UPVOTE_LOCKOUT_HF17, "Cannot increase payout within last twelve hours before payout." );
         else if( _db.has_hardfork( OFFER_HARDFORK_0_7 ) )
            FC_ASSERT( _db.head_block_time() < _db.calculate_discussion_payout_time( comment ) - OFFER_UPVOTE_LOCKOUT_HF7, "Cannot increase payout within last minute before payout." );
      }
      ...
      if( !_db.has_hardfork( OFFER_HARDFORK_0_17__769 ) )
      {
         ...
         if( _db.has_hardfork( OFFER_HARDFORK_0_12__177 ) && !_db.has_hardfork( OFFER_HARDFORK_0_13__257)  )
            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + OFFER_CASHOUT_WINDOW_SECONDS_PRE_HF17;
         else
            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + OFFER_CASHOUT_WINDOW_SECONDS_PRE_HF12;
      ...
      _db.modify( comment, [&]( comment_object& c ){
         ...
         if( rshares > 0 )
            ...
            c.net_votes--;
         if( !_db.has_hardfork( OFFER_HARDFORK_0_6__114 ) && c.net_rshares == -c.abs_rshares) FC_ASSERT( c.net_votes < 0, "Comment has negative net votes?" );
      });
      ...
      _db.modify( root, [&]( comment_object& c )
      {
         ...
         if( !_db.has_hardfork( OFFER_HARDFORK_0_17__769 ) )
         {
            if( _db.has_hardfork( OFFER_HARDFORK_0_12__177 ) && c.last_payout > fc::time_point_sec::min() )
            ...
            if( c.max_cashout_time == fc::time_point_sec::maximum() )
               c.max_cashout_time = _db.head_block_time() + fc::seconds( OFFER_MAX_CASHOUT_WINDOW_SECONDS );
         }
      });
      ...
      **/
      _db.create<comment_vote_object>( [&]( comment_vote_object& cv ){
         ...
         if( curation_reward_eligible && _db.has_hardfork( OFFER_HARDFORK_0_17__774 ) )
            curation_reward_eligible = _db.get_curation_rewards_percent( comment ) > 0;

         if( curation_reward_eligible )
         {
            if( comment.created < fc::time_point_sec(OFFER_HARDFORK_0_6_REVERSE_AUCTION_TIME) ) {
               ...
               if( _db.has_hardfork( OFFER_HARDFORK_0_17__774 ) )
               {
                  ...
                  auto curve = !_db.has_hardfork( OFFER_HARDFORK_0_19__1052 ) && comment.created > OFFER_HF_19_SQRT_PRE_CALC
            ...
            if( _db.head_block_time() > fc::time_point_sec(OFFER_HARDFORK_0_6_REVERSE_AUCTION_TIME) )  /// start enforcing this prior to the hardfork
            {
   ...
   {
      FC_ASSERT( itr->num_changes < OFFER_MAX_VOTE_CHANGES, "Voter has used the maximum number of vote changes on this comment." );

      if( _db.has_hardfork( OFFER_HARDFORK_0_6__112 ) )
      ...
      if( !_db.has_hardfork( OFFER_HARDFORK_0_17__769 ) )
      {
         ...
         if( _db.has_hardfork( OFFER_HARDFORK_0_14__259 ) && abs_rshares == 0 )
...
void hf20_vote_evaluator( const vote_operation& o, database& _db )
{
   ...
   auto now = _db.head_block_time();
   FC_ASSERT( ( now - voter.last_vote_time ).to_seconds() >= OFFER_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );
   ...
   uint128_t used_mana = ( uint128_t( voter.voting_manabar.current_mana ) * abs_weight * 60 * 60 * 24 ) / OFFER_100_PERCENT;
   ...
   if( cashout_delta < OFFER_UPVOTE_LOCKOUT_SECONDS )
   {
      abs_rshares = (int64_t) ( ( uint128_t( abs_rshares ) * cashout_delta ) / OFFER_UPVOTE_LOCKOUT_SECONDS ).to_uint64();
   }
...
{ try {
   if( _db.has_hardfork( OFFER_HARDFORK_0_20__2539 ) )
   {
...
void custom_evaluator::do_apply( const custom_operation& o )
{
   ...
   if( _db.is_producing() || _db.has_hardfork( OFFER_HARDFORK_0_20 ) )
   {
      FC_ASSERT( o.required_auths.size() <= OFFER_MAX_AUTHORITY_MEMBERSHIP, "Too many auths specified. Max: 10, Current: ${n}", ("n", o.required_auths.size()) );
   }
}
...
void custom_json_evaluator::do_apply( const custom_json_operation& o )
{
   ...
   if( _db.is_producing() || _db.has_hardfork( OFFER_HARDFORK_0_20 ) )
   {
      ...
      FC_ASSERT( num_auths <= OFFER_MAX_AUTHORITY_MEMBERSHIP, "Too many auths specified. Max: 10, Current: ${n}", ("n", num_auths) );
   }
...
void custom_binary_evaluator::do_apply( const custom_binary_operation& o )
{
   ...
   FC_ASSERT( d.has_hardfork( OFFER_HARDFORK_0_14__317 ) );
...
void pow_apply( database& db, Operation o )
{
   ...
   if( db.has_hardfork( OFFER_HARDFORK_0_5__59 ) )
   {
   ...
   if( db.has_hardfork( OFFER_HARDFORK_0_13__256 ) )
   ...
   if( db.head_block_num() < OFFER_START_MINER_VOTING_BLOCK )
      pow_reward.amount *= OFFER_MAX_WITNESSES;
...
void pow_evaluator::do_apply( const pow_operation& o ) {
   FC_ASSERT( !db().has_hardfork( OFFER_HARDFORK_0_13__256 ), "pow is deprecated. Use pow2 instead" );
   pow_apply( db(), o );
}
...
{
   database& db = this->db();
   FC_ASSERT( !db.has_hardfork( OFFER_HARDFORK_0_17__770 ), "mining is now disabled" );

   const auto& dgp = db.get_dynamic_global_properties();
   uint32_t target_pow = db.get_pow_summary_target();
   account_name_type worker_account;

   if( db.has_hardfork( OFFER_HARDFORK_0_16__551 ) )
   {
   ...
   FC_ASSERT( o.props.maximum_block_size >= OFFER_MIN_BLOCK_SIZE_LIMIT * 2, "Voted maximum block size is too small." );
...
{
   if( _db.has_hardfork( OFFER_HARDFORK_0_20__409 ) )
      FC_ASSERT( is_asset_type( o.exchange_rate.base, BSD_SYMBOL ) && is_asset_type( o.exchange_rate.quote, OFFER_SYMBOL ),
            "Price feed must be a BSD/OFFER price" );
...
void convert_evaluator::do_apply( const convert_operation& o )
{
  ...
  auto offer_conversion_delay = OFFER_CONVERSION_DELAY_PRE_HF_16;
  if( _db.has_hardfork( OFFER_HARDFORK_0_16__551) )
     offer_conversion_delay = OFFER_CONVERSION_DELAY;
  ...
  _db.create<convert_request_object>( [&]( convert_request_object& obj )
  {
      ...
      obj.amount          = o.amount;
      obj.conversion_date = _db.head_block_time() + offer_conversion_delay;
  });
...
void limit_order_create_evaluator::do_apply( const limit_order_create_operation& o )
{
   ...
   if( _db.has_hardfork( OFFER_HARDFORK_0_20__1449) )
   {
      FC_ASSERT( o.expiration <= _db.head_block_time() + OFFER_MAX_LIMIT_ORDER_EXPIRATION, "Limit Order Expiration must not be more than 28 days in the future" );
   }
   ...
   const auto& order = _db.create<limit_order_object>( [&]( limit_order_object& obj )
   {
       ...
       else
       {
          ...
          obj.expiration = std::min( o.expiration, fc::time_point_sec( OFFER_HARDFORK_0_20_TIME + OFFER_MAX_LIMIT_ORDER_EXPIRATION + rand_offset ) );
       }
   });
...
void limit_order_create2_evaluator::do_apply( const limit_order_create2_operation& o )
{
   ...
   const auto& order = _db.create<limit_order_object>( [&]( limit_order_object& obj )
   {
       ...
       {
          obj.expiration = std::min( o.expiration, fc::time_point_sec( OFFER_HARDFORK_0_20_TIME + OFFER_MAX_LIMIT_ORDER_EXPIRATION ) );
       }
   });
...
{
   FC_ASSERT( !_db.has_hardfork( OFFER_HARDFORK_0_4 ), "report_over_production_operation is disabled." );
}

void claim_account_evaluator::do_apply( const claim_account_operation& o )
{
   FC_ASSERT( _db.has_hardfork( OFFER_HARDFORK_0_20__1771 ), "claim_account_operation is not enabled until hardfork 20." );
   ...
   if( o.fee.amount == 0 )
   {
      ...
      if( !_db.is_pending_tx() )
      {
         ...
         FC_ASSERT( current_witness.available_witness_account_subsidies >= OFFER_ACCOUNT_SUBSIDY_PRECISION, "Witness ${w} does not have enough subsidized accounts to claim",
            ("w", current_witness.owner) );

         _db.modify( current_witness, [&]( witness_object& w )
         {
            w.available_witness_account_subsidies -= OFFER_ACCOUNT_SUBSIDY_PRECISION;
         });
      }

      FC_ASSERT( gpo.available_account_subsidies >= OFFER_ACCOUNT_SUBSIDY_PRECISION, "There are not enough subsidized accounts to claim" );

      _db.modify( gpo, [&]( dynamic_global_property_object& gpo )
      {
         gpo.available_account_subsidies -= OFFER_ACCOUNT_SUBSIDY_PRECISION;
      });
   }
   else
   {
   ...
   _db.adjust_balance( _db.get_account( OFFER_NULL_ACCOUNT ), o.fee );
...
{
   FC_ASSERT( _db.has_hardfork( OFFER_HARDFORK_0_20__1771 ), "create_claimed_account_operation is not enabled until hardfork 20." );
...
void request_account_recovery_evaluator::do_apply( const request_account_recovery_operation& o )
{
   ...
   if ( account_to_recover.recovery_account.length() )   // Make sure recovery matches expected recovery account
   {
      ...
      if( o.recovery_account == OFFER_TEMP_ACCOUNT )
   ...
   if( request == recovery_request_idx.end() ) // New Request
   {
      ...
      _db.create< account_recovery_request_object >( [&]( account_recovery_request_object& req )
      {
         ...
         req.expires = _db.head_block_time() + OFFER_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
      });
   }
...
void recover_account_evaluator::do_apply( const recover_account_operation& o )
{
   ...
   if( _db.has_hardfork( OFFER_HARDFORK_0_12 ) )
      FC_ASSERT( _db.head_block_time() - account.last_account_recovery > OFFER_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
...
void change_recovery_account_evaluator::do_apply( const change_recovery_account_operation& o )
{
   ...
   if( request == change_recovery_idx.end() ) // New request
   {
      _db.create< change_recovery_account_request_object >( [&]( change_recovery_account_request_object& req )
      {
         ...
         req.effective_on = _db.head_block_time() + OFFER_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
...
void transfer_from_savings_evaluator::do_apply( const transfer_from_savings_operation& op )
{
   ...
   FC_ASSERT( from.savings_withdraw_requests < OFFER_SAVINGS_WITHDRAW_REQUEST_LIMIT, "Account has reached limit for pending withdraw requests." );
   ...
   _db.create<savings_withdraw_object>( [&]( savings_withdraw_object& s ) {
      ...
      s.complete = _db.head_block_time() + OFFER_SAVINGS_WITHDRAW_TIME;
   });
...
{
   FC_ASSERT( _db.has_hardfork( OFFER_HARDFORK_0_14__324 ) );
   ...
   if( o.decline )
   {
      ...
      _db.create< decline_voting_rights_request_object >( [&]( decline_voting_rights_request_object& req )
      {
         ...
         req.effective_date = _db.head_block_time() + OFFER_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
   else
   {
...
void claim_reward_balance_evaluator::do_apply( const claim_reward_balance_operation& op )
{
   ...
   FC_ASSERT( op.reward_offer <= acnt.reward_offer_balance, "Cannot claim that much OFFER. Claim: ${c} Actual: ${a}",
      ("c", op.reward_offer)("a", acnt.reward_offer_balance) );
   ...
   asset reward_coining_offer_to_move = asset( 0, OFFER_SYMBOL );
   if( op.reward_coins == acnt.reward_coining_balance )
      reward_coining_offer_to_move = acnt.reward_coining_offer;
   else
      reward_coining_offer_to_move = asset( ( ( uint128_t( op.reward_coins.amount.value ) * uint128_t( acnt.reward_coining_offer.amount.value ) )
         / uint128_t( acnt.reward_coining_balance.amount.value ) ).to_uint64(), OFFER_SYMBOL );

   _db.adjust_reward_balance( acnt, -op.reward_offer );
   _db.adjust_reward_balance( acnt, -op.reward_bsd );
   _db.adjust_balance( acnt, op.reward_offer );
   ...
   _db.modify( acnt, [&]( account_object& a )
   {
      ...
      a.reward_coining_offer -= reward_coining_offer_to_move;
   });

   _db.modify( _db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
   {
      gpo.total_coining_shares += op.reward_coins;
      gpo.total_coining_fund_offer += reward_coining_offer_to_move;

      gpo.pending_rewarded_coining_shares -= op.reward_coins;
      gpo.pending_rewarded_coining_offer -= reward_coining_offer_to_move;
   });
      ...
      else
      {
         ...
         if( token.symbol == COINS_SYMBOL)
         {
            ...
            if( token == a->reward_coining_balance )
               reward_coining_offer_to_move = a->reward_coining_offer;
            else
               reward_coining_offer_to_move = asset( ( ( uint128_t( token.amount.value ) * uint128_t( a->reward_coining_offer.amount.value ) )
                  / uint128_t( a->reward_coining_balance.amount.value ) ).to_uint64(), OFFER_SYMBOL );
         ...
         else if( token.symbol == OFFER_SYMBOL || token.symbol == BSD_SYMBOL )
         {
            FC_ASSERT( is_asset_type( token, OFFER_SYMBOL ) == false || token <= a->reward_offer_balance,
                       "Cannot claim that much OFFER. Claim: ${c} Actual: ${a}", ("c", token)("a", a->reward_offer_balance) );
...
#pragma message( "TODO: Update get_effective_coining_shares when modifying this operation to support SMTs." )
   ...
   if( _db.has_hardfork( OFFER_HARDFORK_0_20__2539 ) )
   {
      ...
      {
         util::manabar_params params( max_mana, OFFER_VOTING_MANA_REGENERATION_SECONDS );
         a.voting_manabar.regenerate_mana( params, _db.head_block_time() );
      });
   ...
   auto min_delegation = _db.has_hardfork( OFFER_HARDFORK_0_20__1761 ) ?
      asset( wso.median_props.account_creation_fee.amount / 3, OFFER_SYMBOL ) * gpo.get_coining_share_price() :
      asset( wso.median_props.account_creation_fee.amount * 10, OFFER_SYMBOL ) * gpo.get_coining_share_price();
   auto min_update = _db.has_hardfork( OFFER_HARDFORK_0_20__1761 ) ?
      asset( wso.median_props.account_creation_fee.amount / 30, OFFER_SYMBOL ) * gpo.get_coining_share_price() :
   ...
   else if( op.coining_shares >= delegation->coining_shares )
   {
      ...
      FC_ASSERT( delta >= min_update, "Offer Power increase is not enough of a difference. min_update: ${min}", ("min", min_update) );
   ...
   else /* delegation->coining_shares > op.coining_shares */
   {
      ...
      {
         FC_ASSERT( delta >= min_update, "Offer Power decrease is not enough of a difference. min_update: ${min}", ("min", min_update) );
