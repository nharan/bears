...
using offer::protocol::chain_id_type;
...
{
   class variant;
   inline void to_variant( const offer::chain::shared_string& s, variant& var )
   {
      var = fc::string( offer::chain::to_string( s ) );
   }

   inline void from_variant( const variant& var, offer::chain::shared_string& s )
   {
   ...
   namespace raw {
      ...
      template< typename T >
      inline T unpack_from_vector( const offer::chain::buffer_type& s )
      { try  {
         T tmp;
         if( s.size() ) {
...
FC_REFLECT_ENUM( offer::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_authority_object_type)
                 (witness_object_type)
...
#ifndef ENABLE_STD_ALLOCATOR
FC_REFLECT_TYPENAME( offer::chain::shared_string )
#endif

FC_REFLECT_ENUM( offer::chain::bandwidth_type, (post)(forum)(market) )
