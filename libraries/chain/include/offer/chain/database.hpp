 ...
 */
#pragma once
#include <offer/chain/block_log.hpp>
#include <offer/chain/fork_database.hpp>
#include <offer/chain/global_property_object.hpp>
#include <offer/chain/hardfork_property_object.hpp>
#include <offer/chain/node_property_object.hpp>
#include <offer/chain/notifications.hpp>

#include <offer/chain/util/advanced_benchmark_dumper.hpp>
#include <offer/chain/util/signal.hpp>
...
namespace offer { namespace chain {
   ...
   class database : public chainbase::database
   {
      public:
         ...
         struct open_args
         {
            ...
            uint64_t initial_supply = OFFER_INIT_SUPPLY;
         ...
         chain_id_type offer_chain_id = OFFER_CHAIN_ID;
      ...
      public:
          ...
          *
          * Passing slot_num == 0 returns OFFER_NULL_WITNESS
          */
         ...
         /** @return the bsd created and deposited to_account, may return OFFER if there is no median feed */
         std::pair< asset, asset > create_bsd( const account_object& to_account, asset offer, bool to_reward_balance=false );
         asset create_coining( const account_object& to_account, asset offer, bool to_reward_balance=false );
          ...
          * OFFER.  Return 0 BSD if there isn't a current_median_history
          */
         asset to_bsd( const asset& offer )const;
         asset to_offer( const asset& bsd )const;
         ...
         void init_schema();
         void init_genesis(uint64_t initial_supply = OFFER_INIT_SUPPLY );
   ...
   protected:
      ...
      private:
         ...
         fc::time_point_sec            _hardfork_times[ OFFER_NUM_HARDFORKS + 1 ];
         protocol::hardfork_version    _hardfork_versions[ OFFER_NUM_HARDFORKS + 1 ];
