...
namespace offer { namespace plugins { namespace chain {

using namespace offer;
using fc::flat_map;
using offer::chain::block_id_type;
...
const offer::chain::database& chain_plugin::db() const { return my->db; }
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   ...
   cli.add_options()
         ...
         ("chain-id", bpo::value< std::string >()->default_value( OFFER_CHAIN_ID ), "chain ID to connect to")
         ;
}
...
void chain_plugin::plugin_startup()
{
   ...
   {
      auto statsd = appbase::app().find_plugin< offer::plugins::statsd::statsd_plugin >();
      if( statsd != nullptr )
      {
   ...
   offer::utilities::benchmark_dumper dumper;

   const auto& abstract_index_cntr = my->db.get_abstract_index_cntr();

   typedef offer::utilities::benchmark_dumper::index_memory_details_cntr_t index_memory_details_cntr_t;
   ...
   db_open_args.initial_supply = OFFER_INIT_SUPPLY;
      ...
      const chainbase::database::abstract_index_cntr_t& abstract_index_cntr )
   {
      ...
      {
         typedef offer::utilities::benchmark_dumper::database_object_sizeof_cntr_t database_object_sizeof_cntr_t;
      ...
      const offer::utilities::benchmark_dumper::measurement& measure =
   ...
   if(my->replay)
   {
      ...
      db_open_args.benchmark = offer::chain::database::TBenchmark(my->benchmark_interval, benchmark_lambda);
      last_block_number = my->db.reindex( db_open_args );

      if( my->benchmark_interval > 0 )
      {
         const offer::utilities::benchmark_dumper::measurement& total_data = dumper.dump(true, get_indexes_memory_details);
   ...
   {
      db_open_args.benchmark = offer::chain::database::TBenchmark(dump_memory_details, benchmark_lambda);
...
bool chain_plugin::accept_block( const offer::chain::signed_block& block, bool currently_syncing, uint32_t skip )
{
...
void chain_plugin::accept_transaction( const offer::chain::signed_transaction& trx )
{
   boost::promise< void > prom;
   write_context cxt;
   cxt.req_ptr = &trx;
...
offer::chain::signed_block chain_plugin::generate_block(
   const fc::time_point_sec when,
   const account_name_type& witness_owner,
   const fc::ecc::private_key& block_signing_private_key,
   uint32_t skip )
...
bool chain_plugin::block_is_on_preferred_chain(const offer::chain::block_id_type& block_id )
{
   ...
   return db().get_block_id_for_num( offer::chain::block_header::num_from_id( block_id ) ) == block_id;
}

void chain_plugin::check_time_in_block( const offer::chain::signed_block& block )
{
...
} } } // namespace offer::plugis::chain::chain_apis
