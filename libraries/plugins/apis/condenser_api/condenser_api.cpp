...
namespace detail
{
      ...
      public:
            ...
            _db( _chain.db() )
         {
            _on_post_apply_block_conn = _db.add_post_apply_block_handler(
               ...
               appbase::app().get_plugin< offer::plugins::condenser_api::condenser_api_plugin >(),
               0 );
         }
   ...
   {
      FC_ASSERT( false, "condenser_api::get_account_references --- Needs to be refactored for Offer." );
   }
...
{
   if( a.voting_manabar.last_update_time < OFFER_HARDFORK_0_20_TIME )
   ...
   // vp_t2 = OFFER_100_PERCENT * current_mana / coins
   // vp_t1 = vp_t2 - OFFER_100_PERCENT * (t2 - t1) / OFFER_VOTING_MANA_REGENERATION_SECONDS
   //

   uint32_t t1 = a.last_vote_time.sec_since_epoch();
   uint32_t t2 = a.voting_manabar.last_update_time;
   uint64_t dt = (t2 > t1) ? (t2 - t1) : 0;
   uint64_t vp_dt = OFFER_100_PERCENT * dt / OFFER_VOTING_MANA_REGENERATION_SECONDS;

   uint128_t vp_t2 = OFFER_100_PERCENT;
   vp_t2 *= a.voting_manabar.current_mana;
   vp_t2 /= coins;

   uint64_t vp_t2u = vp_t2.to_uint64();
   if( vp_t2u >= OFFER_100_PERCENT )
   {
      wlog( "Truncated vp_t2u to OFFER_100_PERCENT for account ${a}", ("a", a.name) );
      vp_t2u = OFFER_100_PERCENT;
   }
...
{
   JSON_RPC_REGISTER_API( OFFER_CONDENSER_API_PLUGIN_NAME );
}
