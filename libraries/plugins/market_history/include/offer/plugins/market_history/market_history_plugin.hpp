...
// time.
//
#ifndef OFFER_MARKET_HISTORY_SPACE_ID
#define OFFER_MARKET_HISTORY_SPACE_ID 7
#endif

#ifndef OFFER_MARKET_HISTORY_PLUGIN_NAME
#define OFFER_MARKET_HISTORY_PLUGIN_NAME "market_history"
#endif
...
{
   bucket_object_type        = ( OFFER_MARKET_HISTORY_SPACE_ID << 8 ),
   order_history_object_type = ( OFFER_MARKET_HISTORY_SPACE_ID << 8 ) + 1
};
...
class market_history_plugin : public plugin< market_history_plugin >
{
   public:
      ...
      static const std::string& name() { static std::string name = OFFER_MARKET_HISTORY_PLUGIN_NAME; return name; }
...
struct bucket_object : public object< bucket_object_type, bucket_object >
{
   ...
   bucket_object_details offer;
   bucket_object_details non_offer;

#ifdef OFFER_ENABLE_SMT
   asset_symbol_type symbol = BSD_SYMBOL;

   price high()const { return asset( non_offer.high, symbol ) / asset( offer.high, OFFER_SYMBOL ); }
   price low()const { return asset( non_offer.low, symbol ) / asset( offer.low, OFFER_SYMBOL ); }
#else
   price high()const { return asset( non_offer.high, BSD_SYMBOL ) / asset( offer.high, OFFER_SYMBOL ); }
   price low()const { return asset( non_offer.low, BSD_SYMBOL ) / asset( offer.low, OFFER_SYMBOL ); }
#endif
};
...
FC_REFLECT( offer::plugins::market_history::bucket_object_details,
            (high)
            (low)
            (open)
            (close)
...
#if defined OFFER_ENABLE_SMT
FC_REFLECT( offer::plugins::market_history::bucket_object,
                     (id)
                     (open)(seconds)
                     (offer)(symbol)(non_offer)
         )
#else
FC_REFLECT( offer::plugins::market_history::bucket_object,
                     (id)
...
CHAINBASE_SET_INDEX_TYPE( offer::plugins::market_history::bucket_object, offer::plugins::market_history::bucket_index )

FC_REFLECT( offer::plugins::market_history::order_history_object,
                     (id)
                     (time)
                     (op) )
CHAINBASE_SET_INDEX_TYPE( offer::plugins::market_history::order_history_object, offer::plugins::market_history::order_history_index )
