...
#include <offer/plugins/witness/witness_export_objects.hpp>
...
namespace detail {
   ...
   public:
      witness_plugin_impl( boost::asio::io_service& io ) :
         _timer(io),
         _chain_plugin( appbase::app().get_plugin< offer::plugins::chain::chain_plugin >() ),
      ...
      uint32_t _required_witness_participation  = 33 * OFFER_1_PERCENT;
      uint32_t _production_skip_flags           = chain::database::skip_nothing;
      bool     _skip_enforce_bandwidth          = true;

      std::map< offer::protocol::public_key_type, fc::ecc::private_key > _private_keys;
      std::set< offer::protocol::account_name_type >                     _witnesses;
      boost::asio::deadline_timer                                        _timer;

      std::set< offer::protocol::account_name_type >                     _dupe_customs;
   ...
   struct comment_options_extension_visitor
   {
      ...
      {
         OFFER_ASSERT( cpb.beneficiaries.size() <= 8,
            plugin_exception,
   ...
   void check_memo( const string& memo, const chain::account_object& account, const account_authority_object& auth )
   {
      ...
      {
         for( auto& key : keys )
            OFFER_ASSERT( key_weight_pair.first != key,  plugin_exception,
      ...
      for( auto& key : keys )
         OFFER_ASSERT( memo_key != key,  plugin_exception,
   ...
   struct operation_visitor
   {
      ...
      void operator()( const comment_operation& o )const
      {
         if( o.parent_author != OFFER_ROOT_POST_PARENT )
         {
            ...
            if( parent != nullptr )
            OFFER_ASSERT( parent->depth < OFFER_SOFT_MAX_COMMENT_DEPTH,
               plugin_exception,
               "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",OFFER_SOFT_MAX_COMMENT_DEPTH) );
         }
      }
      ...
      switch( note.op.which() )
      {
         ...
         case operation::tag< custom_binary_operation >::value:
         {
            ...
            for( auto& account : impacted )
               if( _db.is_producing() )
                  OFFER_ASSERT( _dupe_customs.insert( account ).second, plugin_exception,
   ...
   { try {
      ...
      if( BOOST_UNLIKELY( reserve_ratio_ptr == nullptr ) )
      {
         _db.create< reserve_ratio_object >( [&]( reserve_ratio_object& r )
         {
            ...
            r.current_reserve_ratio = OFFER_MAX_RESERVE_RATIO * RESERVE_RATIO_PRECISION;
            r.max_virtual_bandwidth = ( static_cast<uint128_t>( OFFER_MAX_BLOCK_SIZE) * OFFER_MAX_RESERVE_RATIO
                                       * OFFER_BANDWIDTH_PRECISION * OFFER_BANDWIDTH_AVERAGE_WINDOW_SECONDS )
                                       / OFFER_BLOCK_INTERVAL;
         });
            ...
            */
            if( _db.head_block_num() % 20 == 0 )
            {
               ...
               else
               {
                  ...
                  if( r.current_reserve_ratio > OFFER_MAX_RESERVE_RATIO * RESERVE_RATIO_PRECISION )
                     r.current_reserve_ratio = OFFER_MAX_RESERVE_RATIO * RESERVE_RATIO_PRECISION;
               }
               ...
               r.max_virtual_bandwidth = ( uint128_t( max_block_size ) * uint128_t( r.current_reserve_ratio )
                                          * uint128_t( OFFER_BANDWIDTH_PRECISION * OFFER_BANDWIDTH_AVERAGE_WINDOW_SECONDS ) )
                                          / ( OFFER_BLOCK_INTERVAL * RESERVE_RATIO_PRECISION );
            }
         });
      }

      std::shared_ptr< exp_witness_data_object > export_data =
         offer::plugins::block_data_export::find_export_data< exp_witness_data_object >( OFFER_WITNESS_PLUGIN_NAME );
      if( export_data )
   ...
   void witness_plugin_impl::update_account_bandwidth( const chain::account_object& a, uint32_t trx_size, const bandwidth_type type )
   {
      ...
      if( props.total_coining_shares.amount > 0 )
      {
         ...
         share_type trx_bandwidth = trx_size * OFFER_BANDWIDTH_PRECISION;
         auto delta_time = ( _db.head_block_time() - band->last_bandwidth_update ).to_seconds();

         if( delta_time > OFFER_BANDWIDTH_AVERAGE_WINDOW_SECONDS )
            new_bandwidth = 0;
         else
            new_bandwidth = ( ( ( OFFER_BANDWIDTH_AVERAGE_WINDOW_SECONDS - delta_time ) * fc::uint128( band->average_bandwidth.value ) )
               / OFFER_BANDWIDTH_AVERAGE_WINDOW_SECONDS ).to_uint64();
         ...
         if( ( !_db.has_hardfork( OFFER_HARDFORK_0_20 ) ||  !_skip_enforce_bandwidth ) && _db.is_producing() )
         {
            OFFER_ASSERT( has_bandwidth,  plugin_exception,
               "Account: ${account} bandwidth limit exceeded. Please wait to transact or power up OFFER.",
               ("account", a.name)
   ...
   {
      if( fc::time_point::now() < fc::time_point(OFFER_GENESIS_TIME) )
      {
         wlog( "waiting until genesis time to produce block: ${t}", ("t",OFFER_GENESIS_TIME) );
   ...
   block_production_condition::block_production_condition_enum witness_plugin_impl::maybe_produce_block(fc::mutable_variant_object& capture)
   {
      ...
      {
         capture("pct", uint32_t(100*uint64_t(prate) / OFFER_1_PERCENT));
      ...
      appbase::app().get_plugin< offer::plugins::p2p::p2p_plugin >().broadcast_block( block );
...
{ try {
   ...
   if( export_plugin != nullptr )
   {
      ...
      export_plugin->register_export_data_factory( OFFER_WITNESS_PLUGIN_NAME,
         []() -> std::shared_ptr< exportable_block_data > { return std::make_shared< exp_witness_data_object >(); } );
   }

   OFFER_LOAD_VALUE_SET( options, "witness", my->_witnesses, offer::protocol::account_name_type )
   ...
   if( options.count("private-key") )
   {
      ...
      {
         fc::optional<fc::ecc::private_key> private_key = offer::utilities::wif_to_key(wif_key);
   ...
   {
      my->_required_witness_participation = OFFER_1_PERCENT * options.at( "required-participation" ).as< uint32_t >();
   }
...
{ try {
   ...
   chain::database& d = appbase::app().get_plugin< offer::plugins::chain::chain_plugin >().db();

   if( !my->_witnesses.empty() )
   {
      ilog( "Launching block production for ${n} witnesses.", ("n", my->_witnesses.size()) );
      appbase::app().get_plugin< offer::plugins::p2p::p2p_plugin >().set_block_production( true );
