...
#include <offer/plugins/rc/rc_curve.hpp>
#include <offer/plugins/rc/rc_export_objects.hpp>
...
#include <offer/jsonball/jsonball.hpp>

#define OFFER_RC_REGEN_TIME   (60*60*24*5)
...
using offer::chain::util::manabar_params;
...
void create_rc_account( database& db, uint32_t now, const account_object& account, asset max_rc_creation_adjustment )
{
   ...
   if( max_rc_creation_adjustment.symbol == OFFER_SYMBOL )
   {
   ...
   rc_plugin_skip_flags skip )
{
   if( account_name == account_name_type() )
   {
      if( db.is_producing() )
      {
         OFFER_ASSERT( false, plugin_exception,
   ...
   mbparams.regen_time = OFFER_RC_REGEN_TIME;
   ...
   db.modify( rc_account, [&]( rc_account_object& rca )
   {
      ...
      if( (!skip.skip_reject_not_enough_rc) && db.has_hardfork( OFFER_HARDFORK_0_20 ) )
      {
         if( db.is_producing() )
         {
            OFFER_ASSERT( has_mana, plugin_exception,
               "Account: ${account} has ${rc_current} RC, needs ${rc_needed} RC. Please wait to transact, or power up OFFER.",
...
void rc_plugin_impl::on_post_apply_transaction( const transaction_notification& note )
{
   ...
   int64_t rc_regen = (gpo.total_coining_shares.amount.value / (OFFER_RC_REGEN_TIME / OFFER_BLOCK_INTERVAL));
   ...
   if( rc_regen > 0 )
   {
      for( size_t i=0; i<OFFER_NUM_RESOURCE_TYPES; i++ )
      {
   ...
   std::shared_ptr< exp_rc_data > export_data =
      offer::plugins::block_data_export::find_export_data< exp_rc_data >( OFFER_RC_PLUGIN_NAME );
      ...
      [&]( rc_pool_object& pool_obj )
      {
         ...
         for( size_t i=0; i<OFFER_NUM_RESOURCE_TYPES; i++ )
         {
            ...
            if( debug_print )
            {
               ...
               a /= k*double(pool);
               dlog( "a=${a}   aR=${aR}", ("a", a)("aR", a*gpo.total_coining_shares.amount.value/OFFER_RC_REGEN_TIME) );
            }
         }
         if( debug_print )
         {
...
void rc_plugin_impl::on_first_block()
{
   ...
   std::string resource_params_json = offer::jsonball::get_resource_parameters();
...
struct pre_apply_operation_visitor
{
   ...
   void regenerate( const account_object& account, const rc_account_object& rc_account )const
   {
      ...
      // TODO:  Issue number
      //
      static_assert( OFFER_RC_REGEN_TIME <= OFFER_VOTING_MANA_REGENERATION_SECONDS, "RC regen time must be smaller than vote regen time" );
   ...
   {
      if( op.hardfork_id == OFFER_HARDFORK_0_1 )
      {
         const auto& idx = _db.get_index< account_index >().indices().get< by_id >();
         for( auto it=idx.begin(); it!=idx.end(); ++it )
         {
...
struct post_apply_operation_visitor
{
   ...
   void operator()( const pow_operation& op )const
   {
      ...
      create_rc_account< true >( _db, _current_time, op.worker_account, asset( 0, OFFER_SYMBOL ) );
   ...
   void operator()( const pow2_operation& op )const
   {
      ...
      create_rc_account< true >( _db, _current_time, worker_name, asset( 0, OFFER_SYMBOL ) );
   ...
   void operator()( const hardfork_operation& op )const
   {
      ...
      if( op.hardfork_id == OFFER_HARDFORK_0_20 )
      {
         ...
         _db.modify( _db.get< rc_pool_object, by_id >( rc_pool_object::id_type() ), [&]( rc_pool_object& p )
         {
            ...
            {
               p.pool_array[ i ] =
                  ( ( uint128_t( params.resource_param_array[ i ].resource_dynamics_params.pool_eq ) * 90 * OFFER_1_PERCENT )
                  / OFFER_100_PERCENT ).to_uint64();
            }
   ...
   void operator()( const clear_null_account_balance_operation& op )const
   {
      _mod_accounts.push_back( OFFER_NULL_ACCOUNT );
   }
...
void rc_plugin_impl::on_pre_apply_operation( const operation_notification& note )
{
   ...
   if( _db.has_hardfork( OFFER_HARDFORK_0_20 ) )
...
void rc_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   try
   {
      ...
      if( export_plugin != nullptr )
      {
         ...
         export_plugin->register_export_data_factory( OFFER_RC_PLUGIN_NAME,
