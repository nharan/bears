...
namespace detail
{
   ...
   {
      public:
         blockchain_statistics_api_impl( offer::app::application& app )
            :_app( app ) {}
   ...
   statistics blockchain_statistics_api_impl::get_stats_for_interval( fc::time_point_sec start, fc::time_point_sec end )const
   {
      ...
      const auto& sizes = _app.get_plugin< blockchain_statistics_plugin >( OFFER_BLOCKCHAIN_STATISTICS_PLUGIN_NAME )->get_tracked_buckets();
...
blockchain_statistics_api::blockchain_statistics_api( const offer::app::api_context& ctx )
{
...
statistics& statistics::operator +=( const bucket_object& b )
{
   ...
   this->offer_transferred                      += b.offer_transferred;
   ...
   this->offer_coined                           += b.offer_coined;
   ...
   this->offer_converted                        += b.offer_converted;
