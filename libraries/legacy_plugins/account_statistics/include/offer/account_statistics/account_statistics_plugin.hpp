...
// time.
//
#ifndef OFFER_ACCOUNT_STATISTICS_SPACE_ID
#define OFFER_ACCOUNT_STATISTICS_SPACE_ID 10
#endif

#ifndef OFFER_ACCOUNT_STATISTICS_PLUGIN_NAME
#define OFFER_ACCOUNT_STATISTICS_PLUGIN_NAME "account_stats"
#endif
...
{
   account_stats_bucket_object_type    = ( OFFER_ACCOUNT_STATISTICS_SPACE_ID << 8 ),
   account_activity_bucket_object_type = ( OFFER_ACCOUNT_STATISTICS_SPACE_ID << 8 ) + 1
};
...
struct account_stats_bucket_object : public object< account_stats_bucket_object_type, account_stats_bucket_object >
{
   ...
   share_type           author_rewards_total_offer_value = 0;     ///< OFFER Value of author rewards
   ...
   share_type           curation_rewards_offer_value = 0;         ///< OFFER Value of curation rewards
   ...
   share_type           liquidity_rewards = 0;                    ///< Amount of OFFER paid as liquidity rewards
   ...
   share_type           offer_sent = 0;                           ///< OFFER sent from this account
   share_type           offer_received = 0;                       ///< OFFER received by this account
   ...
   share_type           offer_coined = 0;                         ///< OFFER coined by the account
   ...
   share_type           offer_received_from_withdrawls = 0;       ///< OFFER received from this account's coining withdrawals
   share_type           offer_received_from_routes = 0;           ///< OFFER received from another account's coining withdrawals
   ...
   share_type           limit_order_offer_paid = 0;               ///< OFFER paid by limit orders
   share_type           limit_order_offer_received = 0;           ///< OFFER received from limit orders
...
class account_statistics_plugin : public offer::app::plugin
{
   public:
      account_statistics_plugin( application* app );
      virtual ~account_statistics_plugin();

      virtual std::string plugin_name()const override { return OFFER_ACCOUNT_STATISTICS_PLUGIN_NAME; }
...
FC_REFLECT( offer::account_statistics::account_stats_bucket_object,
   (id)
   (open)
   (seconds)
   (name)
   ...
   (coins_withdrawn)
   (offer_received_from_withdrawls)
   (offer_received_from_routes)
   ...
   (limit_order_offer_paid)
   (limit_order_offer_received)
   (limit_order_bsd_paid)
   (limit_order_bsd_received)
   (total_pow)
   (estimated_hashpower)
)
//SET_INDEX_TYPE( offer::account_statistics::account_stats_bucket_object,)

FC_REFLECT(
   offer::account_statistics::account_activity_bucket_object,
   (id)
   (open)
   (seconds)
