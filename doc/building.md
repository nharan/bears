...
Builds offerd to be a consensus-only low memory node. Data and fields not
...
### BUILD_OFFER_TESTNET=[OFF/ON]

Builds offer for use in a private testnet. Also required for building unit tests.
...
For Ubuntu 16.04 users, after installing the right packages with `apt` Offer
...
Offer requires Boost 1.58 (as in Ubuntu 16.04) and works with versions up to 1.60 (including).
So building Offer on Ubuntu 14.04 requires downloading and installing a more recent
version of Boost.
...
Ubuntu 14.04) when compiling in C++11 mode (which Offer does).
...
offer. Until then, this will allow you to install boost 1.60.0.
...
e.g.:
    ...
    make -j$(sysctl -n hw.logicalcpu) offerd

This will only build `offerd`.
