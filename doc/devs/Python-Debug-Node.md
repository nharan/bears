...
of a running Offer Debug Node. The Debug Node is a plugin for Offer that allows realtime
...
[Python Offer library](https://github.com/xeroc/python-offerlib).
...
To use the script include `from offerdebugnode import DebugNode`

There are a couple of examples already made that you can try modifying yourself.

[debug_hardforks.py](https://github.com/offerit/offer/python_scripts/tests/debug_hardforks.py)
...
[debugnode.py](https://github.com/offerit/offer/python_scripts/offerdebugnode/debugnode.py#L212)
...
``` Python
debug_node = DebugNode( str( offerd ), str( data_dir ) )
with debug_node:
...
specify the offerd binary location and a populated data directory. For most configurations
this will be `programs/offerd/offerd` and `witness_node_data_dir` respectively, from the
git root directory for Offer.
