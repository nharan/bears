...
Anyone (even you!) can edit their account to contain 1 million OFFER.  It's really
...
yourself 1 million OFFER.  Just like it's really impossible for your bank to physically stop
...
the fake OFFER or fake dollars you "gave" yourself).  So you can believe whatever you want
about your balance and rewrite the rules of your own bookkeeping system to show you whatever balance you
want to be shown, but as soon as you try to actually spend OFFER (or dollars) that you don't actually have,
...
(see `libraries/plugins/debug_node/include/offer/plugins/debug_node/debug_node_api.hpp`
...
Okay, let's run `offerd`.  It should start immediately with no blocks.  We can ask it to read blocks from the directory we saved earlier:
...
If we want to take control of an account we can do so by editing its key with `debug_update_object` command like this:
    ...
    curl --data '{"jsonrpc": "2.0", "method": "call", "params": [0,"lookup_account_names",[["offerit"]]], "id": 8}' http://127.0.0.1:6990/rpc    # find out ID of account we want is 2.2.28
...
Now that we've reset its key, we can take control of it in the wallet:
    ...
    list_my_accounts
    transfer offerit dantheman "1.234 OFFER" "make -j100 money" true
    list_my_accounts
    get_account_history offerit -1 1000
